...
;;; Desc.  : Code to read in an OPS5 program from file and create 
...
;;; BEGIN_EUSCHEME
;;  (import (level0 reader-ce reader-vars prod-gf prod 
;;                cond-el-gf cond-el-1 reader-act ops-out))
;;
;;  (defun pprint (x)
;;    (format ops-out "~a~%" x))
;;  (defun end-of-input (next)
;;    (string-size  (convert (car next) <string>))
;;    (if (atom next) ()
;;      (equal (convert (car next) <string>) "**EOF**")))
;;  (defun eos-default-value ()
;;    '**EOS**)
;;  (defun symbol-name (x)
;;     (convert x <string>))
;;  (defconstant read-next read)
;;  (defun open-ops-file (x)
;;    (make <input-port> filename: x))
;;
;;; END_EUSCHEME
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (print "### reader.em")
