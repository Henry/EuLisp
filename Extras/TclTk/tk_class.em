...
(defgeneric tk-xview ((object <tk-object>) (type <string>) (entry <string>) units))
(defgeneric tk-yview ((object <tk-object>) (type <string>) (entry <string>) units))
  ...
  (if (and parent (null (tk-object-p parent)))
    ...
    (let (x)
      ...
      (and (tk-object-p parent)
           (put-on x parent))
      ((setter tk-handler) x 
       (eul_tk_create_widget (symbol-name type) (tk-name x) (as-c-options options x)))
      x)))
;;--------------------------------------------------------------------------;;
;;                                                                          ;;
...
(defmethod tk-delete ((entry <tk-entry>) (first <string>) . last)
  (and (or (and (equal (list-size last) 1)
                (stringp (car last)))
           (null last))
       (eul_delete_command (tk-name entry) (tk-handler entry) first last)))
(defmethod tk-xview ((entryWidget <tk-entry>) (type <string>) (entry <string>) units)
  (eul_view_cmd (tk-name entryWidget) (tk-handler entryWidget) type entry units "xview"))
...
(defmethod tk-insert ((listbox <tk-listbox>) (type <string>) (num  <int>))
  (eul_insert_command (tk-name listbox) (tk-handler listbox) type (convert num <string>)))
(defmethod tk-delete ((listbox <tk-listbox>) (first <string>) . last)
  (and (or (and (equal (list-size last) 1)
                (stringp (car last)))
           (null last))
       (eul_delete_command (tk-name listbox) (tk-handler listbox) first last)))
  
(defmethod tk-xview ((listbox <tk-listbox>) (type <string>) (entry <string>) units)
  (eul_view_cmd (tk-name listbox) (tk-handler listbox) type entry units "xview"))
  
(defmethod tk-yview ((listbox <tk-listbox>) (type <string>) (entry <string>) units)
  (eul_view_cmd (tk-name listbox) (tk-handler listbox) type entry units "yview"))
(defun tk-listbox-curselection (listbox)
  (and (tk-listbox-p listbox)
       (eul_tk_listbox_cmd (tk-name listbox) (tk-handler listbox) "curselection")))
;;--------------------------------------------------------------------------;;
;;                                                                          ;;
;;                         Functions for Menu                               ;;
;;                                                                          ;;
...
(defmethod tk-menu-add ((menu <tk-menu>) (type <symbol>) . options)
  (let ((newoptions (cons (symbol-name type) options)))
    (eul_add_menu_command (tk-name menu) (tk-handler menu) (as-c-options newoptions menu))))
;;--------------------------------------------------------------------------;;
;;                                                                          ;;
...
(defmethod tk-insert ((text-widget <tk-text>) (type <string>) (text <string>))
  (eul_insert_command (tk-name text-widget) (tk-handler text-widget) type text))
...
(defmethod tk-get-value-widget ((x <tk-text>) . indexs)
  ;; Due to we are calling a C foreign function we need to say how many arguments
  ;; There are in this list. The way this foreign function works needs to have the
  ;; quantity of elements in the last argument.
  (let ((size (list-size indexs)))
    (and  (or (equal size 1)
              (equal size 2))
          (eul_tk_get_value_widget (tk-name x) (tk-handler x) (cons size indexs)))))
;;--------------------------------------------------------------------------;;
;;                                                                          ;;
