...
LispRef *eul_dyn_create_module(char *module_name, int size)
{
    ...
    binding_vec = (LispRef *) gc_malloc(size * sizeof(LispRef *));
    ...
    eul_fast_table_set(eul_modules, as_C_module_name(module_name),
    (LispRef) binding_vec);
...
int load_compiled_module(char *module_name, FILE * fp)
{
    ...
    binding_vec = create_binding_vector(module_name, fp);

    /* Get bytevectors */
    while (is_init_bv == 0)
    {
        size = create_next_bytevector(&bv, fp, binding_vec);
...
int create_next_bytevector(LispRef * bv_ptr, FILE * fp, LispRef * binding_vec)
{
    ...
    fscanf(fp, "with size: %d is_init: %d index: %d binding: %s */\n",
    &size, &is_init_bv, &index, buf);

    byte_size = sizeof(LispRef) * size;
...
int eul_dyn_load_module(char *module_name, char *file_name)
{
    ...
    res = load_compiled_module(module_name, fp);
    fclose(fp);
    if (res)
...
(LispRef bv, int bv_index, char *module_name, int local_index)
{
