                   ...
                   (if (memq s '(... ___))
                     ...
                     (let* ((s (symbol->string s))
                       ...
                       (and (<= 3 n)
                            ...
                            (match:andmap char-numeric? (string->list
                                                         (substring s 2 n)))
                                                      ...
                                                      (if (equal? (car p) 'set!)
                                                        ...
                                                        (if (equal? (car p) 'get!)
                                                          ...
                                                          (if (equal? (car p) 'unquote)
                                                              (g88 (car p) (cdr p))
                                                            (if (equal? (car p) 'unquote-splicing)
                                                                (g88 (car p) (cdr p))
                                                              (if (and (pair? (cdr p))
                                                                       (dot-dot-k? (cadr p))
                                                                       (null? (cddr p)))
                ...
                (let* ((v '())
                  ...
                  (let next ((p (caar plist))
                       ...
                       (cond
                         ...
                         ((and (vector? p)
                               (>= (vector-length p) 6)
                               (dot-dot-k? (vector-ref p (- (vector-length p)
                                                            5))))
                                   ...
                                   (let ((x (car l)))
                                     (or (and (equal?
                                              ...
                                              (not (equal?
                                                    const-class
                                                    (car x))))
                                         (equal? x `(not (,const-class
                                                          ,(cadr srch))))
                                         (and (equal? (cadr x) (cadr srch))
                                              (equal-test? x)
                                              (not (equal? (caddr srch)
                                                           (caddr x))))
                                         (mem (cdr l)))))))
                           ((disjoint? srch)
                            (let mem ((l l))
          ...
          (let g229 ((g230 (cdddr args)) (g228 '()))
               (if (null? g230)
                 ...
                 (if (field? (car g230))
                   ...
                   (g227))))
        (g227)))))
(defmacro define-structure args
  (let ((g242 (lambda ()
                (match:syntax-err
    ...
    (if (and (pair? args)
        ...
        (if (null? (cdr args))
          ...
          (if (and (pair? (cdr args)) (list? (cadr args)))
            ...
            (g242)))
      (g242))))
(defmacro define-const-structure args
  (let ((field? (lambda (id)
                  (if (symbol? id)
                      ((lambda () #t))
                    (if (and (pair? id)
