...
* Introduction
  ...
  most notable being a full metaobject system which is implemented in Youtoo not
  in EuXLisp.  These are a few notes on the use of EuLisp in general and EuXLisp
  in particular and concentrate on those things unique to EuLisp.
...
* Assignment
  ...
  be used at the top level (i.e., never inside a function definition or a =let=).
...
* Classes and Generic Functions
  ...
  to functions, returned from functions, and so on.  For example, the integer =1=
  has class =<integer>= (or rather, has a class with name =<integer>=).
  ...
  + =requiredp:= use =requiredp: t= to indicate a required slot.  This slot must
  ...
  + =constructor:= a way to name a function to make an instance of the class.
      ...
      instance of =<rat>=.  The first argument will be given to the =num:= keyword,
      the second to the =den:=.  This is equivalent to defining
      #+BEGIN_SRC eulisp
      (defun rat (n d)
        (make <rat> num: n den: d))
      #+END_SRC
      As usual, you can reorder or leave out bits as you feel.
  + =abstractp: t= to indicate that this class is abstract, and no direct
...
*** Setters
    ...
    #+END_SRC
    Similarly, the accessor =car= has an updater =(setter car)= (often called =rplaca=
    in other Lisps), and so on.  In general a reader function =r= will have an
    associated updater =(setter r)=.
    ...
    ((setter setter) r w)
    #+END_SRC
    In fact, no particular properties of =r= and =w=, are used, so this can be used
    as a general facility.  Further, setter functions, generic functions and
    methods can be defined directly:
    #+BEGIN_SRC eulisp
...
*** Paralation Lisp
    ...
    lisp) exports
    + =(make-paralation n)= to make a new paralation of size =n=.  This returns a
      index field of the new paralation.
...
*** Shell Scripts
    ...
    (print "hello world")
    #+END_SRC
    It is usual to use the =-q= flag to prevent the echo from the
    read-eval-print loop, and the =-s= flag to prevent the use of the system
    function.
    #+BEGIN_SRC eulisp
    #!/usr/local/bin/euxlisp -qs --script
