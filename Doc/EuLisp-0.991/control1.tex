...
by \identifier{} \index{general}{binding!top dynamic}.  The binding created by
...
\rewriterules
%
\savesyntax\catchRewrite\vbox{\tt%
\begin{tabular}{lll}%
    (\macroref{catch}) & \rewrite &
        {\rm Is an error}\\
    (\macroref{catch} {\em tag}) & \rewrite &
        (\keywordref{progn} {\em tag} ())\\
    (\macroref{catch} {\em tag} \forms) & \rewrite &
    \begin{minipage}[t]{0.3\columnwidth}
    \begin{tabbing}
        00\=00\= \kill
        (\keywordref{let/cc} tmp\\
        \>(\keywordref{dynamic-let} (({\em tag} tmp))\\
        \>\>\forms))
    \end{tabbing}
    \end{minipage}
\end{tabular}
}
\rewritetable{catch}{\catchRewrite}
...
\rewriterules
%
\savesyntax\throwRewrite\vbox{\tt%
\begin{tabular}{lll}
    (\macroref{throw}) & \rewrite &
        {\rm Is an error}\\
    (\macroref{throw} {\em tag}) & \rewrite &
        ((\keywordref{dynamic} {\em tag}) ())\\
    (\macroref{throw} {\em tag} \form) & \rewrite &
        ((\keywordref{dynamic} {\em tag}) \form)
\end{tabular}
}
\rewritetable{throw}{\throwRewrite}
%
\seealso%
\macroref{catch}.
%
