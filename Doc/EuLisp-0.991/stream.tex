...
\begin{initoptions}
    ...
    \classref{stream} \genericref{generic-read} method. The accessor for this slot is called
    {\tt stream-read-action}.
    %
    \item[write-action, \classref{function}] A function which is called by the
    \classref{stream} \genericref{generic-write} method. The accessor for this slot
    is called {\tt stream-write-action}.
\end{initoptions}
%
...
\begin{functions}
    ...
    %
    \item[stream-buffer-size] The maximum number of objects that can be stored in
    {\em stream-buffer}. Its default value is 0.
\end{functions}
%
\noindent
...
\function{to-stream}
% ------------------------------------------------------------------------------
A constructor function of one argument for \classref{stream} which returns a stream
...
%
This class specializes \classref{fixed-buffered-stream} by providing an interface to data
stored in files on disk. The transaction unit of \classref{file-stream} is
\classref{character}. The following additional accessor functions are defined for
...
\classref{string-stream}\ to \classref{string}.
...
% ------------------------------------------------------------------------------
\begin{genericargs}
    \item[source, \classref{object}] The source object from which the stream will
    read data.
    \item[sink, \classref{object}] The sink object to which the stream will write
    data.
    \item[\optional{options}, \classref{list}] An optional argument for specifying
    implementation-defined options.
\end{genericargs}
%
\remarks%
...
% ------------------------------------------------------------------------------
\begin{specargs}
    \item[source, \classref{stream}] The stream which is to be the source of {\em
        sink}.
...
directly), thus a \genericref{read} operation on \instanceref{lispin} will case
...
the function \functionref{end-of-stream} is called with the argument {\em
    stream}.
...
the function \functionref{fill-buffer} is called. If the refilling operation did
...
%
\seealso%
Methods on \genericref{generic-write} are defined for
\classref{character}, \classref{double-float}, \classref{fixed-precision-integer},
\classref{list}, \classref{string}, \classref{symbol} and \classref{vector}.
