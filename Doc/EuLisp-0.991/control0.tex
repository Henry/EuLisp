...
%
\Syntax
\savesyntax\defconstantSyntax\vbox{\small\syntax
defconstant form
...
%
\Syntax
\savesyntax\deflocalSyntax\vbox{\small\syntax
deflocal form
...
representation~\verb+'+\ttindex{quote!abbreviation with {\tt '}}---so that {\tt
...
%
\Syntax
\savesyntax\defmacroSyntax\vbox{\small\syntax
defmacro form
...
%
\remarks%
The \defformref{defmacro} form defines a function\index{general}{macro} named by
{\em macro-name} and stores the definition as the top-lexical binding of {\em
    macro-name}\index{general}{macro!definition by
    \defformref{defmacro}}\index{general}{binding!module}.  The interpretation
...
%
\Syntax
\savesyntax\defunSyntax\vbox{\small\syntax
defun form
   = simple defun
   | setter defun;
simple defun
...
{\em lambda-list} is as defined for \keywordref{lambda}.  The rewrite rules for
\syntaxref{defun} are given below.
%
\savesyntax\defunRewrites\vbox{\begin{center}
{\small\tt\begin{tabular}{lll}
&&\\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(de\=fun {\em identifier}\\
   \>{\em lambda-list}\\
   \>{\em body})
\end{tabbing}
\end{minipage}
...
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(de\=fconstant {\em identifier}\\
   \>(lambda {\em lambda-list} {\em body}))
\end{tabbing}
\end{minipage}\\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(de\=fun\\
   \>(setter {\em identifier})\\
   \>{\em lambda-list} {\em body})
\end{tabbing}
\end{minipage}
& $\equiv$ &
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
((s\=etter setter) {\em identifier}\\
   \>(lambda {\em lambda-list} {\em body}))
\end{tabbing}
\end{minipage}\\
&&\\
\end{tabular}}
\end{center}}
\syntaxtable{defun {\normalfont rewrite rules}}{\defunRewrites}

\metasyntaxform{function call}
\index{general}{function!calling}
%
\Syntax
\savesyntax\functioncallSyntax\vbox{\small\syntax
function call form
   = '(', operator, {operand}, ')';
\endsyntax}
...
%
\Syntax
\savesyntax\ifSyntax\vbox{\small\syntax
if form
...
%
\Syntax
\savesyntax\condSyntax\vbox{\small\syntax
cond macro
   = '(', 'cond',
...
The \macroref{cond} macro provides a convenient syntax for collections of {\em
    if-then-elseif...else} expressions.  The rewrite rules for \macroref{cond}
are given below.
%
\savesyntax\condRewrites\vbox{\begin{center}
{\small{\tt
\begin{tabular}{lll}
&&\\
(cond) & $\equiv$ & () \\

(cond ({\em antecedent}) \ldots) & $\equiv$ & (or {\em antecedent} (cond \ldots)) \\

%(cond (t {\em form}$^*$)) & $\equiv$ & (progn {\em form}$^*$) \\
...
\end{center}}
\syntaxtable{cond {\normalfont rewrite rules}}{\condRewrites}
...
%
\remarks%
The expansion of an \macroref{and} form leads to the evaluation of the
...
non-strict.  The result of {\tt (\macroref{and})} is \true.  If none of the {\em
form}s evaluate to \nil, the value of the last {\em form} is
returned.  The rewrite rules for \macroref{and} are given below.
%
\savesyntax\andRewrites\vbox{\begin{center}
{\small\tt
\begin{tabular}{lll}
&&\\
(and) & $\equiv$ & t \\
(and {\em form}) & $\equiv$ & {\em form} \\
(and {\em form}$_1$ {\em form}$_2$ \ldots) & $\equiv$ &
\begin{minipage}[t]{0.45\columnwidth}\begin{tabbing}
(if \={\em form}$_1$\\
    \>(and {\em form}$_2$ \ldots)\\
    \>())
\end{tabbing}\end{minipage}\\
&&\\
\end{tabular}}
\end{center}}
\syntaxtable{and {\normalfont rewrite rules}}{\andRewrites}

\macro{or}
%
\Syntax
\savesyntax\orSyntax\vbox{\small\syntax
or macro
   = '(', 'or', {form}, ')';
\endsyntax}
...
of {\em form}s from left to right.  The value of the first {\em form} that
...
If none of the forms evaluate to {\em true}, the value of the last {\em form} is
returned.  The rewrite rules for \macroref{or} are given below.  Note that {\tt
    x} does not occur free in any of {\em form}$_2$ \ldots {\em form}$_n$.
%
\savesyntax\orRewrites\vbox{\begin{center}
{\small\tt
\begin{tabular}{lll}
&&\\
(or) & $\equiv$ & () \\
(or {\em form}) & $\equiv$ & {\em form} \\
(or {\em form}$_1$ {\em form}$_2$ \ldots) & $\equiv$ &
\begin{minipage}[t]{0.45\columnwidth}\begin{tabbing}
(le\=t ((x {\em form}$_1$))\\
   \>(if \=x\\
         \>x\\
...
\end{tabular}}
\end{center}}
\syntaxtable{or {\normalfont rewrite rules}}{\orRewrites}
%
\end{optDefinition}
...
\keyword{let/cc}
\ttindex{let/cc!see also \macroref{block} and \macroref{return-from}}
%
\Syntax
\savesyntax\letccSyntax\vbox{\small\syntax
let/cc form
...
\macro{block}
%
\ttindex{block!see also \keywordref{let/cc}}
\Syntax
\savesyntax\blockSyntax\vbox{\small\syntax
block macro
...
as the value of the block form.  See also \keywordref{let/cc}.  The rewrite
rules for \macroref{block} are given below.
% in Table~\ref{block-return-from-rules}.
...
to {\em identifier} is a first-class item and can be passed as an
argument like other values.
%
\savesyntax\blockRewrites\vbox{\begin{center}
{\small\tt
\begin{tabular}{lll}
&&\\
%(block) & $\equiv$ & {\rm Is an error}\\
(block {\em identifier}) & $\equiv$ & ()\\
(block {\em identifier} {\em form}$^*$) & $\equiv$ & (let/cc {\em identifier} {\em form}$^*$) \\
&&\\
\end{tabular}}
\end{center}}
\syntaxtable{block {\normalfont rewrite rules}}{\blockRewrites}
%
\seealso%
\macroref{return-from}.

\macro{return-from}
\ttindex{return-from!see also \keywordref{let/cc}}
%
\Syntax
\savesyntax\returnfromSyntax\vbox{\small\syntax
return from macro
...
%
\remarks%
In \macroref{return-from}, the {\em identifier} names the continuation of
...
%if the value of the variable named by {\em identifier} is not a
%continuation.
\macroref{return-from} is the invocation of the continuation of the block
named by {\em identifier}.  The {\em form} is evaluated and the value
is returned as the value of the block named by {\em identifier}.  The
rewrite rules for \macroref{return-from} are given below.
%
\savesyntax\returnFromRewrites\vbox{\begin{center}
{\small\tt
\begin{tabular}{lll}
&&\\
%(return-from) & $\equiv$ & {\rm Is an error}\\
(return-from {\em identifier}) & $\equiv$ & ({\em identifier} ())\\
\begin{minipage}[t]{0.45\columnwidth}\begin{tabbing}
(re\=turn-from\\
   \>{\em identifier} {\em form})
\end{tabbing}\end{minipage}
& $\equiv$ & ({\em identifier} {\em form})\\
&&\\
\end{tabular}}
\end{center}}
\syntaxtable{return-from {\normalfont rewrite rules}}{\returnFromRewrites}
%
\seealso%
\macroref{block}.
...
%
\Syntax
\savesyntax\labelsSyntax\vbox{\small\syntax
labels form
   = '(', 'labels',
          '(', {function definition}, ')',
          {form}, ')';
...
%
\Syntax
\savesyntax\letSyntax\vbox{\small\syntax
let macro
...
the \macroref{let} form.  The rewrite rules for \macroref{let} are given below.
%
\savesyntax\letRewrites\vbox{\begin{center}
{\small{\tt
\begin{tabular}{lll}
(let () {\em form}$^*$) & $\equiv$ & (progn {\em form}$^*$) \\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(l\=et (\=({\em id}$_1$ {\em form}$_1$) \\
  \>    \>({\em id}$_2$ {\em form}$_2$) \\
  \>    \>{\em id}$_3$ \\
  \>    \>\ldots)\\
  \>{\em form}$^*$)
...
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(\=(l\=ambda ({\em id}$_1$ {\em id}$_2$ {\em id}$_3$ \ldots) \\
 \>  \>{\em form}$^*$) \\
 \>{\em form}$_1$ {\em form}$_2$ () \ldots) \\
\end{tabbing}
\end{minipage}\\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
...
\end{tabular}}}
\end{center}}
\syntaxtable{let {\normalfont rewrite rules}}{\letRewrites}

\macro{let*}
%
\Syntax
\savesyntax\letstarSyntax\vbox{\small\syntax
let star macro
   = '(', 'let*', '(', {binding}, ')', {form}, ')';
\endsyntax}
...
form.  The rewrite rules for \macroref{let*} are given below.
%
\savesyntax\letStarRewrites\vbox{\begin{center}
{\small\tt
\begin{tabular}{lll}
(let* () {\em form}$^*$) & $\equiv$ & (progn {\em form}$^*$) \\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
(l\=et* (\=({\em var}$_1$ {\em form}$_1$) \\
  \>     \>({\em var}$_2$ {\em form}$_2$) \\
  \>     \>{\em var}$_3$\\
  \>     \>\ldots) \\
  \>{\em form}$^*$)\\
\end{tabbing}
...
\end{center}}
\syntaxtable{let* {\normalfont rewrite rules}}{\letStarRewrites}
...
and a \defformref{defmodule} form, then the {\em form}s within the
...
%The {\em form} is evaluated and the elements of {\em lambda-list} are
...
%15.  If {\em lambda-list} is an {\em identifier}, it is bound to a
%newly allocated list of the multiple values.  If {\em lambda-list}
%is a {\em rest-list}, each {\em identifier} preceding the dot is bound
...
(\verb+`+)\ttindex{quasiquote!abbreviation with {\tt `}}, so that {\tt
    (\macroref{quasiquote} {\em expression})} can be written {\tt `{\em
        expression}}.
%A complete definition of the syntax of quasiquote
%expressions is given in Table~\ref{quasiquote-syntax}.
...
\syntaxref{unquote-splice} the {\em form} must result in a proper list.  The
...
(\verb+,+)\ttindex{unquote!abbreviation with {\tt ,}} preceding an expression
and by using the diphthong {\em comma} followed by the glyph called {\em
    commercial at} (\verb+,@+)\ttindex{unquote-splicing!abbreviation to ,@}
