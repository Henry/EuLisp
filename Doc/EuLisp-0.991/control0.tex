...
returned.  The rewrite rules for \macroref{or} are given below:

\savesyntax\orRewrites\vbox{%
{\small\tt%
\begin{tabular}{lll}%
(or) & $\equiv$ & () \\
...
Note that {\tt x} does not occur free in any of {\em form}$_2$ \ldots {\em
    form}$_n$.
%
\end{optDefinition}
...
\savesyntax\returnFromRewrites\vbox{%
{\small\tt%
\begin{tabular}{lll}%
...
\savesyntax\letStarRewrites\vbox{%
{\small\tt%
\begin{tabular}{lll}%
(let* () {\em form}$^*$) & $\equiv$ & (progn {\em form}$^*$) \\
\begin{minipage}[t]{\columnwidth}
