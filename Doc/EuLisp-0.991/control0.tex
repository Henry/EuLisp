...
%
\remarks%
The result of processing the expression {\tt (quote \scref{object})} is
\scref{object}.  The \scref{object} can be any object having an external
...
must be a \scref{rest-list}.  In this case, each \scref{identifier} preceding the
dot is bound to the corresponding argument and the \scref{identifier} succeeding
...
\scref{macro-name} \index{general}{macro!definition by \syntaxref{defmacro}}
...
%
\remarks%
The \syntaxref{defun} form defines a function named by \scref{function-name} and
...
\begin{minipage}[t]{\columnwidth}%
    \begin{tabbing}%
        00\= \kill
        (\syntaxref{defun} \scref{identifier}\\
        \>\scref{lambda-list}\\
        \>\scref{body})
    \end{tabbing}
\end{minipage}
...
\begin{arguments}
    ...
    \scref{atomic-expression}, a \scref{literal-expression} or a
    \scref{function-call-form}.
\end{arguments}
%
\result%
...
\begin{RewriteTable}{cond}{lll}
    ...
    (\macroref{cond} (\scref{antecedent}) \\
    \tts\ldots) &\rewrite&
    (\macroref{or} \scref{antecedent} (\macroref{cond} \ldots)) \\
\begin{minipage}[t]{0.45\columnwidth}
\begin{tabbing}
    00\=00\= \kill
    (\macroref{cond} \\
    \>(\scref{antecedent}$_1$) \\
    \>(\scref{antecedent}$_2$ \scseqref{form}) \\
    \>\ldots)
\end{tabbing}
\end{minipage}
&\rewrite&
\begin{minipage}[t]{0.45\columnwidth}
\begin{tabbing}
    (\macroref{or} \= \scref{antecedent}$_1$ \\
    \>00\= \kill
    \>(\macroref{cond} \\
    \>\>(\scref{antecedent}$_2$ \scseqref{form}) \\
    \>\>\ldots))
...
\begin{tabbing}
    00\=00\= \kill
    (\macroref{cond} \\
    \>(\scref{antecedent}$_1$ \scseqref{form}) \\
    \>(\scref{antecedent}$_2$ \scseqref{form}) \\
    \>\ldots)
\end{tabbing}
\end{minipage}
...
of \scref{form}s from left to right.  The first \scref{form} in the sequence that
evaluates to \nil{}\/ stops evaluation and none of the \scref{form}s to its right
will be evaluated---that is to say, it is non-strict.  The result of {\tt
    (\macroref{and})} is \true{}.  If none of the \scref{form}s evaluate to
...
\begin{RewriteTable}{and}{lll}
    ...
    (\macroref{and} \scref{form}) &\rewrite& \scref{form} \\
    (\macroref{and} \scref{form}$_1$ \scref{form}$_2$ \ldots) &\rewrite&
\begin{minipage}[t]{0.45\columnwidth}
\begin{tabbing}
    (\specopref{if} \= \scref{form}$_1$\\
    \>(\macroref{and} \scref{form}$_2$ \ldots)\\
    \>())
\end{tabbing}%
\end{minipage}%
\end{RewriteTable}
...
If none of the forms evaluate to {\em true}, the value of the last \scref{form} is
returned.
%
\rewriterules
%
\begin{RewriteTable}{or}{lll}
    (\macroref{or}) &\rewrite& () \\
    (\macroref{or} \scref{form}) &\rewrite& \scref{form} \\
    (\macroref{or} \scref{form}$_1$ \scref{form}$_2$ \ldots) &\rewrite&
\begin{minipage}[t]{0.45\columnwidth}
\begin{tabbing}
    (\macroref{let} (\=(x \scref{form}$_1$))\\
    \>(\specopref{if} \= x\\
    \>\>x\\
    \>\>(\macroref{or} \scref{form}$_2$ \ldots)))
\end{tabbing}%
\end{minipage}%
\end{RewriteTable}
%
...
escape function.  The block \scref{variable} is bound to the
continuation\index{general}{continuation} of the block.  The continuation can be
invoked anywhere within the block by using \macroref{return-from}.  The
...
\begin{RewriteTable}{block}{lll}
    (\macroref{block} \scref{identifier}) &\rewrite& ()\\
    (\macroref{block} \scref{identifier}  &\rewrite&
    (\specopref{let/cc} \scref{identifier} \\
    \tts\scseqref{form})                    && \tts\scseqref{form})
\end{RewriteTable}
%
...
%
\remarks%
In \macroref{return-from}, the \scref{identifier} names the continuation of
...
\begin{RewriteTable}{return-from}{lll}
    (\macroref{return-from} \scref{identifier}) &\rewrite& (\scref{identifier} ())\\
\begin{minipage}[t]{0.45\columnwidth}
\begin{tabbing}
    00\= \kill
    (\macroref{return-from} \\
...
identifiers of the \scref{binding}s of the \macroref{let}, whose body is that of
...
\begin{RewriteTable}{let}{lll}
    (\macroref{let} () \scseqref{form}) &\rewrite& (\specopref{progn} \scseqref{form}) \\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
    (\macroref{let} (\=(\scref{id}$_1$ \scref{form}$_1$) \\
    \>(\scref{id}$_2$ \scref{form}$_2$) \\
    \>\scref{id}$_3$ \\
    \>\ldots)\\
    00\= \kill
    \>\scseqref{form})
...
\begin{tabbing}
    00\=00\=00\= \kill
    ((\specopref{lambda} (\scref{id}$_1$ \scref{id}$_2$ \scref{id}$_3$ \ldots) \\
    \>\>\scseqref{form}) \\
...
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
    (\macroref{let} \= \scref{id}$_0$ \\
    \>(\=(\scref{id}$_1$ \scref{form}$_1$) \\
    \>\>\scref{id}$_2$ \\
    \>\>\ldots)\\
    00\= \kill
    \>\scseqref{form})
...
\begin{tabbing}
    00\= \kill
    (\specopref{labels}\\
    \>(\=(\scref{id}$_0$ (\scref{id}$_1$ \scref{id}$_2$ \ldots) \\
    \>\>\scseqref{form})) \\
    \>(\scref{id}$_0$ \scref{form}$_1$ () \ldots))
\end{tabbing}%
\end{minipage}%
\end{RewriteTable}
...
result of the \macroref{let*} form.
%
\rewriterules
%
\begin{RewriteTable}{let*}{lll}
    (\macroref{let*} () \scseqref{form}) &\rewrite& (progn \scseqref{form}) \\
\begin{minipage}[t]{\columnwidth}
\begin{tabbing}
    (\macroref{let*} (\=(\scref{var}$_1$ \scref{form}$_1$) \\
...
\begin{tabbing}%
    00\= \kill
    (\macroref{let} ((\scref{var}$_1$ \scref{form}$_1$)) \\
    \>(\macroref{let*} (\=(\scref{var}$_2$ \scref{form}$_2$) \\
    \>\>\scref{var}$_3$\\
    \>\>\ldots) \\
    00\=00\= \kill
    \>\>\scseqref{form}))
...
(\verb+`+)\ttsubindex{quasiquote}{abbreviation with {\tt `}}, so that {\tt
    (\macroref{quasiquote} \scref{skeleton})} can be written {\tt
    `\scref{skeleton}}.
...
\syntaxref{unquote-splicing} the \scref{form} must result in a proper list.  The
