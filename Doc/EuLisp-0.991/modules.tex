...
\defSyntax{defmodule-0}{
    \begin{syntaxx}
        ...
        \>  export ( \scseqref{indentifier} ) \\
        \>  expose ( \scseqref{module-descriptor} ) \\
        \>  import ( \scseqref{module-descriptor} ) \\
        \>  syntax ( \scseqref{module-descriptor} ) \\
        \scdef{level-0-module-form}: \\
        \>  ( export \scseqref{indentifier} ) \\
        \>  \scref{level-0-expression} \\
        \>  \scref{defining-form} \\
        \>  ( progn \scseqref{level-0-module-form} ) \\
        \scdef{module-descriptor}: \\
        \>  \scref{module-name} \\
        \>  \scref{module-filter} \\
        \scdef{module-filter}: \\
        \>  ( except ( \scseqref{indentifier} ) \scref{module-descriptor} ) \\
        \>  ( only ( \scseqref{indentifier} ) \scref{module-descriptor} ) \\
        \>  ( rename ( \scseqref{rename-pair} ) \scref{module-descriptor} ) \\
        \scdef{rename-pair}: \\
        \>  ( \scref{indentifier} \scref{indentifier} )
    \end{syntaxx}}
