...
\classalias{a-class-alias}{a-class}
%
\remarks%
...
%
\subdefinition{``{\tt -ref}'' suffix} A slot reader is named {\em
    class-name}{\tt{}-ref}---where appropriate---and as is the corresponding
slot writer {\tt (setter {\em class-name}-ref)}---also where appropriate, for
example {\tt table-ref}.  This convention is broken by historical precedent for
the accessors to slots of pairs, which are called \functionref{car} and
\functionref{cdr}.
