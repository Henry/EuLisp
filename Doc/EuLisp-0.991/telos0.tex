...
{\tt
\classref{object} \\
\tts\classref{character} \\
\tts\classref{condition} \\
\tts\tts{\normalfont See Table~\ref{condition-class-hierarchy}}
\tts\classref{function} \\
%NotYoutoo \tts\tts\classref{continuation} \\
\tts\tts\classref{simple-function} \\
\tts\tts\classref{generic-function} \\
\tts\classref{collection} \\
\tts\tts\classref{sequence} \\
\tts\tts\tts\classref{list} \\
\tts\tts\tts\tts\classref{cons} \\
\tts\tts\tts\tts\classref{null}\\
\tts\tts\tts\classref{character-sequence} \\
\tts\tts\tts\tts\classref{string} \\
\tts\tts\tts\classref{vector} \\
\tts\tts\classref{table} \\
\tts\tts\tts\classref{hash-table} \\
\tts\classref{lock} \\
\tts\classref{number} \\
\tts\tts\classref{integer}\\
\tts\tts\tts\classref{fixed-precision-integer}\\
%NotYoutoo \tts\tts\tts\classref{variable-precision-integer}\\
\tts\tts\classref{float}\\
\tts\tts\tts\classref{double-float}\\
\tts\classref{stream} \\
\tts\tts\classref{buffered-stream}\\
\tts\tts\tts\classref{string-stream}\\
\tts\tts\tts\classref{file-stream}\\
\tts\classref{name} \\
\tts\tts\classref{symbol}\\
\tts\tts\classref{keyword}\\
\tts\classref{thread} \\
\tts\tts\classref{simple-thread}\\
}%
...
%\begin{inherits}
%\item[direct-superclass: <object>]
%\item[class-precedence-list: (<class> <object>)] The linearized inheritance
%hierarchy.
%\item[slots: ()]
%\item[initargs: ()]
%\end{inherits}

The default superclass classes. All classes defined using the
...
as follows:
\begin{enumerate}
    \item the {\em CPL\/} of \classref{object}\ is a list of one element containing
    \classref{object}\ itself;
    %
...
%the name to which the new class will be bound.  The second is an
%identifier which names a variable to which the superclass is bound.
%If
%{\em superclass} is \nil, the superclass is taken to be the root
%class \classref{class}.
%The list of {\em
%slot}s is described below.  Finally, a {\em class-option\/}
%is an identifier followed by a corresponding value, which, taken
...
%
\Syntax
\savesyntax\defgenericSyntax\vbox{\small\syntax
defgeneric form
...
\savesyntax\defgenericRewrites\vbox{\small\syntax
...
\macro{defmethod}
\Syntax
\savesyntax\defmethodSyntax\vbox{\small\syntax
defmethod form
...
can be applied immediately, much like the normal \keyword{lambda}.  The {\em
...
class: {\tt <no-next-method>}\indexcondition{no-next-method}) if there
...
class based. \genericref{eql} methods known from \clos\ are excluded.
...
% \genericref{allocate} and initializes it calling
% \genericref{initialize}. \genericref{allocate} discriminates on the class,
% while \genericref{initialize} discriminates on the new object.  Both
% \genericref{initialize} and \genericref{allocate}, as well as more specific
% constructors, are described in more detail in section~\ref{class-0}. The
% default \genericref{allocate} method creates a new uninitialized object, that
% is, all slots are unbound.  The default \genericref{initialize} method (see
% below) is applicable to any object, although there are also more specific
% methods for classes (\ref{class-0}) and for generic functions and methods
% (\ref{gf-0}).
...
%At the metalevel there is a protocol which allows user defined methods
%for the computation of special accessors, for example, those checking
%the type of a value when storing a new one.
