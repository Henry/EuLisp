...
\begin{options}
    ...
    only be specified once for a particular slot.  The same \scref{initarg} name
    may be used for several slots, in which case they will share the same
    initial value if the \scref{initarg} is given to \functionref{make}.
    Subclasses inherit the \scref{initarg}. Each slot must have at most one
    \scref{initarg} including the inherited one. That means, a subclass can not
    shadow or add a new \scref{initarg}, if a superclass has already defined
    one.
    ...
    value of the slot, to be used if no \scref{initarg} is defined for the slot
...
\begin{options}
    ...
    corresponding slots if they name a slot \scref{initarg} or are ignored by
    the default \genericref{initialize} method, otherwise. This option must only
    ...
    bound, followed by a sequence of legal \scref{initarg}s for the class.  The
    new function creates an instance of the class and fills in the slots
    according to the match between the specified \scref{initarg}s and the given
...
%
\Signature
\defSignature{abstract-class-p}{
\begin{signature}
    (\functionref{abstract-class-p} \sc{object}) \ra{} \classref{object}
\end{signature}}
\showSignatureBox{abstract-class-p}%
%
\begin{arguments}
    \item[\sc{object}] Returns \sc{object}, if it is an abstract class,
    otherwise \nil{}.
\end{arguments}
%
\begin{note}
...
by \macroref{defmethod}.  An \scref{init-option} is an identifier followed by a
corresponding value.
...
\specop{next-method-p}
\Signature
\defSignature{next-method-p}{
\begin{signature}
    (\specopref{next-method-p}) \ra{} \scref{boolean}
\end{signature}}%
\showSignatureBox{next-method-p}%
%
\result%
If there is a next most specific method, \specopref{next-method-p} returns a
...
\begin{enumerate}
        ...
        \item  If \specopref{call-next-method} appears in one of the method
...
\begin{optRationale}
    ...
    \genericref{initialize}, no check of \scref{initarg}s will be done by the
    system.
...
\begin{enumerate}
    \item Checks if the supplied \scref{initarg}s are legal and signals an error
    otherwise. Legal \scref{initarg}s are those specified in the class
    definition directly or inherited from a superclass.  An \scref{initarg} may
    be specified as a slot-option or as a class-option.

    \item Initializes the slots of the object according to the \scref{initarg},
...
\end{enumerate}
%
Legal \scref{initarg}s which do not initialize a slot are ignored by the default
\genericref{initialize} method.  More specific methods may handle these
\scref{initarg}s and call the default method by calling
\specopref{call-next-method}.
% The default method can be specialized by calling \specopref{call-next-method}
