...
\begin{syntaxx}
    \scdef{defclass-form}: \\
    \>  ( \defformref{defclass} \scref{class-name} \scref{superclass-name} \\
    \>\>  \scref{slot-list} \scseqref{class-option} ) \\
    \scdef{class-name}: \\
    \>  \scref{identifier} \\
    ...
    \>  \scref{identifier} \\
    \scdef{slot-option}: \\
    \>  keyword: \scref{identifier} \\
    \>  default: \scref{level-0-expression} \\
    \>  reader: \scref{identifier} \\
    \>  writer: \scref{identifier} \\
    \>  accessor: \scref{identifier} \\
    \>  requiredp: \scref{boolean} \\
    \scdef{class-option}: \\
    \>  keywords: ( \scseqref{identifier} ) \\
    \>  constructor: \scref{constructor-specification} \\
    \>  predicate: \scref{identifier} \\
    \>  abstractp: \scref{boolean} \\
...
\begin{minipage}[t]{\linewidth}
    \begin{tabbing}
        00\=00\= \kill
        (\defformref{defgeneric} \sc{identifier} \\
        \>\sc{gf-lambda-list} \scseq{level-0-init-option})
    \end{tabbing}
\end{minipage}\\
\hspace{2cm}$\equiv$
\begin{minipage}[t]{\linewidth}
...
\begin{minipage}[t]{\linewidth}
    \begin{tabbing}
        00\=00\= \kill
        ((\functionref{setter} \functionref{setter}) \sc{identifier}\\
        \>(\macroref{generic-lambda} \\
        \>\> \sc{gf-lambda-list} \scseq{level-0-init-option}))
    \end{tabbing}
\end{minipage}\\
...
\begin{minipage}[t]{\linewidth}
    \begin{tabbing}
        00\=00\= \kill
        (\defformref{defgeneric} \\
        \>(\functionref{converter} \sc{identifier}) \\
        \>\sc{gf-lambda-list} \scseq{level-0-init-option})
    \end{tabbing}
\end{minipage}\\
\hspace{2cm}$\equiv$
...
\sc{gf-name}, which is an identifier, or a form denoting a setter function or
...
as for the level-0 definition of \defformref{defgeneric}.  \examples In the
following example an anonymous version of {\tt gf-0} (see
\defformref{defgeneric} above) is defined.  In all other respects the resulting
