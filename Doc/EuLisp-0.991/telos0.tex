...
\begin{syntaxx}
    ...
    \>  \scref{identifier} \\
    \>  ( \scref{identifier} \scseqref{keyword-name} )
\end{syntaxx}}
...
\begin{options}
    ...
    \scref{init-option}s of a call to \functionref{make} on the new class. Initargs
    are inherited by union. The values of all legal arguments in the call to
    \functionref{make} are the initial values of corresponding slots if they
    name a slot initarg or are ignored by the default \genericref{initialize}
...
\begin{arguments}
    ...
    \item[\scref{level-0-init-option}] is \keyworddef{method} \scref{method-description} \\
...
\specopref{call-next-method} if they specialize metaobjects classes and define
...
and call the default method by calling \specopref{call-next-method}.
