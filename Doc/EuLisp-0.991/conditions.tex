...
\savesyntax\defconditionSyntax\vbox{\syntax
defcondition form
   = '(', 'defcondition',
...
%
\remarks%
First calls \keywordref{call-next-method} to carry out initialization specified
...
%
\remarks%
The \keywordref{call-next-handler} special form calls the next enclosing
handler. It is an error to evaluate this form other than within an established
handler function. The \keywordref{call-next-handler} special form is normally
...
a similar but orthogonal way to \keywordref{call-next-method} (assuming a
