...
condition}\index{general}{condition} to be a kind of occasion in a program when
...
\end{enumerate}
%
A condition class is defined using \defformref{defcondition} (see
...
\tts\conditionref{general-condition}\\
\tts\tts\conditionref{invalid-operator}\\
\tts\tts\conditionref{cannot-update-setter}\\
\tts\tts\conditionref{no-setter}\\
\tts\conditionref{environment-condition}\\
\tts\conditionref{arithmetic-condition}\\
\tts\tts \conditionref{division-by-zero}\\
\tts\conditionref{conversion-condition}\\
\tts\tts\conditionref{no-converter}\\
\tts\conditionref{stream-condition}\\
\tts\tts\conditionref{end-of-stream}\\
\tts\conditionref{read-error}\\
\tts\conditionref{thread-condition}\\
\tts\tts\conditionref{thread-already-started}\\
\tts\tts\conditionref{wrong-thread-continuation}\\
\tts\tts\conditionref{wrong-condition-class}\\
\tts\conditionref{generic-function-condition}\\
\tts\tts\conditionref{no-next-method}\\
\tts\tts\conditionref{non-congruent-lambda-lists}\\
\tts\tts\conditionref{incompatible-method-domain}\\
\tts\tts\conditionref{no-applicable-method}\\
\tts\tts\conditionref{method-domain-clash}\\
}%
\end{figure}
\end{optDefinition}
%
...
\begin{optDefinition}
%
\defform{defcondition}
\label{defcondition}
%
\Syntax
\label{defcondition-syntax}
\savesyntax\defconditionSyntax\vbox{\small\syntax
defcondition form
   = '(', 'defcondition',
     ...
     {slot name}, ')';
\endsyntax}
\syntaxtable{defcondition}{\defconditionSyntax}
%
\begin{arguments}
    %
...
taken to be
\classref{condition}.  Otherwise {\em superclass-name\/} must be \classref{condition}\
...
%
\remarks%
First calls \keywordref{call-next-method} to carry out initialization
specified by superclasses then does the \conditionref{condition} specific
initialization.
%
...
%
\begin{initoptions}
    \item[argument, \classref{object}] An argument, which was not of the expected
...
%
\begin{initoptions}
    \item[result, \classref{object}] A result, which was not of the expected class,
    or outside a defined range and therefore lead to the signalling of this
    condition.
\end{initoptions}
...
%
\remarks%
Signalled by one of \defformref{defgeneric}, \macroref{defmethod} or
...
\begin{initoptions}
    ...
    \item[method, method] The method which called \keywordref{call-next-method}.
...
%
\remarks%
Signalled by \keywordref{call-next-method} if there is no next most specific
method.
%
\end{optDefinition}
...
handler}\index{general}{error!handler}.  Handlers are established
...
Handlers are established by the special form \keywordref{with-handler}.
...
%
\seealso%
\functionref{thread-reschedule}, \functionref{thread-value}, \keywordref{with-handler}.

\keyword{call-next-handler}
%
\Syntax
\label{call-next-handler-syntax}
\savesyntax\callnexthandlerSyntax\vbox{\small\syntax
call-next-handler form
   = '(', 'call-next-handler', ')';
\endsyntax}
\syntaxtable{call-next-handler}{\callnexthandlerSyntax}
%
\remarks%
The \keywordref{call-next-handler} special form calls the next
enclosing handler. It is an error to evaluate this form other than within an
established handler function. The \keywordref{call-next-handler} special form is
normally used when a handler function does not know how to deal with the class
of condition. However, it may also be used to combine handler function behaviour
in a similar but orthogonal way to \keywordref{call-next-method} (assuming a
generic handler function).

\keyword{with-handler}
%
\Syntax
\label{with-handler-syntax}
\savesyntax\withhandlerSyntax\vbox{\small\syntax
with-handler form
   = '(', 'with-handler',
     handler function,
     {forms}, ')';
handler function
   = level 0 expression;
\endsyntax}
\syntaxtable{with-handler}{\withhandlerSyntax}
%
\begin{arguments}
    ...
    condition object that was passed to \functionref{signal} as its first argument.  The
    {\em resume} continuation is the continuation (or \nil) that was given
    to \functionref{signal} as its second argument.
    %
    \item[forms] The sequence of forms whose execution is protected
    by the {\em handler function} specified above.
\end{arguments}
%
\result%
...
\remarks%
%
A \keywordref{with-handler} form is evaluated in three steps:
\begin{enumerate}
    ...
    last one is returned as the result of the \keywordref{with-handler}
    expression.
...
\end{enumerate}
%
The above is the normal behaviour of \keywordref{with-handler}.  The exceptional
behaviour of \keywordref{with-handler} happens when there is a call to
...
\begin{enumerate}
    ...
    handler via the \keywordref{call-next-handler} special form.
\end{enumerate}
%
...
initialized from {\em init-option}\/s, the {\em error-message} and a
{\em resume} continuation value of \nil, signifying that the
condition was not signalled continuably.
...
%
\remarks%
The \functionref{cerror} function signals a continuable error.  It
calls \functionref{signal} with an instance of a condition of {\em
    condition-class} initialized from {\em init-option}\/s, the {\em
    error-message} and a {\em resume} continuation value which is the
continuation of the \functionref{cerror} expression.  A non-\nil\/ resume
continuation signifies that the condition has been signalled continuably.
%
\end{optDefinition}
