...
\begin{definitions}
    % \definition{accessor}
    % \index{general}{accessor}\index{general}{slot!accessor} An accessor is a
    % association of a {\em reader} and a {\em writer}.
    %
    ...
    %
    % \definition{applicable method list} \index{general}{applicable method
    %     list} \index{general}{method!list of applicable} An applicable method
    % list is a list of all the {\em methods} applicable for a particular list
    % of arguments to a generic function, sorted according to {\em method} {\em
    %     domain} specificity.
    %
    %% \definition{applicable object}\index{general}{applicable object} An
    %% applicable object is an {\em instance\/} of {\tt function}.
    %
    %% \definition{association list}\index{general}{association list} An
    %% association list is a {\em proper list\/} whose {\tt car} fields contain
    %% {\em objects} of {\em class} {\tt pair}.
    %
    % \definition{binding}\index{general}{binding}
    % A location containing a value.
    %
    % \definition{binding form}\index{general}{binding form}
    % Any form or any {\em macro expression} expanding into a form which causes
    % the creation of {\em inner} {\em dynamic} or {\em inner} {\em lexical}
    % {\em bindings}.
    %
    ...
    \nil.
    %
    % \definition{bound variable}\index{general}{bound variable} A {\em
    %     variable} {\tt x} is bound in an expression $E$ if {\tt x} occurs in
    % the {\em scope} of a {\em defining} form which creates {\em inner-lexical}
    % {\em binding}s or of a {\em binding} {\em form} occurring in $E$ whose
    % variable binding list contains {\tt x}.
    %
    ...
    {\classref{object}}, and all {\em instances} of {\classref{class}} are {\em classes}.
    ...
    % {\tt object}.  This list determines the specificity of slot and method
    % {\em inheritance}.  A class's class precedence list may be accessed
    % through the {\em function} {\tt class-precedence-list}. The rules used to
    % compute this list are determined by the {\em class} of the {\em class}
    % through {\em methods} of the {\em generic} {\em function} {\tt
    %     compute-class-precedence-list}.
    %
    % \definition{class option}\index{general}{class option}
    % A keyword and its associated value applying to a {\em class} appearing in
    % a class definition form, for example: the {\tt predicate} keyword and its
    % value, which defines a predicate {\em function} for the {\em class} being
    % defined.
    %
    % \definition{closure}\index{general}{closure} The closure of an expression
    % $E$ is the set of all {\em free variables} that occur in $E$.
    %
    % \definition{congruent}\index{general}{congruent} A constraint on the form
    % of the lambda-list of a method, which requires it to have the same number
    % of elements as the generic function to which it is to be attached.
    %
    % \definition{constant} A number, string, character or the empty list.
    %
    % \definition{constructor}
    % \index{general}{constructor}\index{general}{class!constructor} A
    % constructor is a {\em function} which creates an {\em instance} of a
    % particular {\em class}.
    %
    ...
    % every point in a program there is the remainder of the program coming
    % after that point.  This can be viewed as a function of one argument
    % awaiting the result of that point.  Such a function is called a
    % continuation.
    %
    ...
    \defformref{defgeneric}, \defformref{deflocal}, \defformref{defmacro},
    \defformref{defun}, or \defformref{defglobal}.
    ...
    % \definition{direct slot description} \index{general}{direct slot
    %     description} \index{general}{slot description!direct} A {\em class's}
    % direct {\em slot} {\em descriptions} are defined specifically for the {\em
    %     class}.
    ...
    % \definition{discrimination} \index{general}{discrimination}
    % \index{general}{generic function!discrimination}
    % \index{general}{method!lookup} {\em Generic} {\em function} application
    % consists of two parts: finding a set of {\em methods} applicable to the
    % given set of arguments, and application of the {\em method} {\em
    %     functions} of those {\em methods}.  The first part is called {\em
    %     discrimination} or {\em method lookup}.  {\em Generic} {\em functions}
    % have an associated {\em function} called the {\em discriminating function}
    % which implements the discrimination.  Users can define new {\em classes}
    % of {\em generic} {\em functions} which implement discrimination in new
    % ways.
    %
    % \definition{domain}\index{general}{domain} A domain is a list of {\em
    %     classes} derived from a list of arguments, or the list of {\em
    %     classes} for which a {\em method} is {\em applicable}.
    ...
    % expression and destroyed when control exits the expression.  Thus the
    % entity only exists for the time between control entering and exiting the
    % expression.
    %
    ...
    %
    % \definition{dynamically closer}\index{general}{dynamically closer} If a
    % form $F2$ is executed in the {\em dynamic extent} of a form $F1$ then
    % within the {\em dynamic extent} of $F2$, $F2$ is dynamically closer than
    % $F1$.
    %
    % \definition{extent}\index{general}{extent} That lifetime for which an
    % entity exists.  Extent is constrained to be either {\em dynamic} or {\em
    %     indefinite}.
    %
    % \definition{form}\index{general}{form} One of {\em constant}, {\em
    %     symbol}, {\em literal}, function call or {\em special form}.
    %
    % \definition{free variable}\index{general}{free variable} A {\em variable}
    % {\tt x} is free in an expression $E$ if {\tt x} does not occur in the {\em
    %     lexical scope} of any {\em defining} which creates {\em inner-lexical}
    % {\em binding}s or any {\em binding} {\em form} occurring in $E$ whose
    % variable binding list contains {\tt x}.
    ...
    %
    % \definition{improper list}\index{general}{improper list} An improper list
    % is a list whose final pair contains something other than the empty list in
    % its {\tt cdr} field.
    %
    % \definition{indefinite extent}\index{general}{indefinite extent} A
    % lifetime constraint, such that the entity exists for ever.  In practice,
    % this means for as long as the entity is accessible.
    %
    % \definition{indefinite scope}\index{general}{indefinite scope} An access
    % constraint, such that the {\em scope} of the {\em variable} is unlimited.
    ...
    % \definition{indirect slot description} \index{general}{indirect slot
    %     description} \index{general}{slot description!indirect} A {\em slot}
    % {\em description} is indirect for a {\em class$_1$} if the {\em slot} {\em
    %     description} is defined for {\em class$_1$}, but was originally
    % defined for another {\em class$_2$} which is a {\em superclass} of {\em
    %     class$_1$} and incorporated into {\em class$_1$} through {\em
    %     inheritance}.  An indirect slot description is also called an {\em
    ...
    graph has a distinguished root, the {\em class} \classref{object}, which is a
    ...
    additional information.  Used in conjunction with \functionref{make} and the other
    ...
    bindings are created by \defformref{dynamic-let}, referenced by
    ...
    bindings are created by \keywordref{lambda} and \keywordref{let/cc}, referenced by {\em
        variables} and modified by \keywordref{setq}.  Inner lexical bindings
    extend---and can shadow---the lexically enclosing {\em lexical environment}.
    Note that \keywordref{let/cc} creates an immutable {\em binding}.
    ...
    one cycle, an edge from \classref{class} to itself.  The instantation graph is
    a tree and \classref{class} is the root.

    % \definition{lexically closer}\index{general}{lexically closer} If a {\em
    %     form} $F2$ occurs in a form $F1$, then any {\em binding}s created by
    % $F2$ are lexically closer than those of $F1$.
    ...
    %
    % \definition{literal}\index{general}{literal} An object created by use of
    % the {\tt quote} operator.
    %
    % \definition{macro}\index{general}{macro} A macro is a function.  A macro
    % is distinguished from a function by when it is used: macro functions are
    % only used during the syntax expansion of modules to transform expressions.
    %
    % \definition{macro expression}\index{general}{macro expression} A form
    % whose operator names a macro expansion function.
    ...
    All metaclasses in \eulisp\ are {\em instances} of \classref{class}, which is
    ...
    % \definition{method-combination}\index{general}{method combination} The
    % applicable method list for an argument list determines the next {\em
    %     method} called by the special form {\tt call-next-method}: for any
    % {\em method} in the list, the next {\em method} is simply the {\em method}
    ...
    % special forms {\tt call-next-method} and {\tt next-method-p} are only
    % valid within the lexical scope of a method function.

    % \definition{method lookup}\index{general}{method lookup} See {\em
    %     discrimination}.
    ...
    % \definition{object} \index{general}{object} Any entity that can be bound
    % to a {\em variable}---including entities from outside \lisp{}'s memory.
    % Every object is an {\em instance} of some {\em class}.
    %
    %% \definition{primitive class}\index{general}{primitive class} A built-in,
    %% {\em implementation-dependent} class which cannot be specialized by the
    %% user.
    %
    % \definition{proper list}\index{general}{proper list} A proper list is a
    % list whose final pair contains the empty list in its {\tt cdr} field, or
    % is just the empty list.
    %
    % \definition{reader} \index{general}{reader}\index{general}{slot!reader} A
    % reader is a {\em function} of one argument which returns the value of a
    % particular {\em slot} in {\em instances} of a particular {\em class}.
    ...
    % \definition{scope}\index{general}{scope} That part of the extent in which
    % a given {\em variable} is accessible.  Scope is constrained to be {\em
    ...
    % \definition{shadow}\index{general}{shadow} If two entities are created for
    % which the same means of reference is used, and either the form creating
    % one occurs lexically in the form creating the other (where the means of
    % reference has {\em lexical scope}) or the form creating one is executed in
    % the dynamic extent of the form creating the other (where the means of
    % reference has {\em dynamic scope}), then the outer entity is shadowed by
    % the inner one.
    ...
    definition form, for example: the \keywordref{accessor} keyword and its
    ...
    % \definition{symbol} \index{general}{symbol} A symbol is a data structure,
    % often used to represent an {\em identifier}.
    ...
    \begin{flushleft} \defformref{defclass}, \defformref{defcondition},
        ...
        \defformref{defmacro}, \defformref{defclass}, \defformref{defun}.
    \end{flushleft}
    ...
    % \definition{variable}\index{general}{variable} A variable denotes a {\em
    %     binding} and is a means to reference the value stored in the {\em
    %     binding}.
    %
    % \definition{writer} \index{general}{writer}\index{general}{slot!writer} A
    % writer is a {\em function} of two arguments which changes the value of a
    % particular {\em slot} in {\em instances} of a particular {\em class}.
\end{definitions}
\end{optDefinition}
