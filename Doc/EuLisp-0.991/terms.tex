...
\begin{definitions}
    ...
    \defopref{defgeneric}, \defopref{deflocal}, \syntaxref{defmacro},
    \syntaxref{defun}, or \defopref{defglobal}.
    ...
    \definition{initarg} \index{general}{initarg} A {\em keyword} used in an
    {\em initlist} to mark the value of some {\em slot} or additional
    information.  Used in conjunction with \functionref{make} and the other {\em
    ...
    \specopref{dynamic} and modified by \specopref{dynamic-setq}.  Inner
    ...
    referenced by {\em variables} and modified by \specopref{setq}.  Inner
    lexical bindings extend---and can shadow---the lexically enclosing {\em
        lexical environment}.  Note that \specopref{let/cc} creates an
    ...
    and modified by \specopref{dynamic-setq}.  There is only one {\em top
        dynamic} environment.
    ...
    \begin{flushleft} \defopref{defclass}, \defopref{defcondition},
        \defopref{defconstant}, \defopref{defgeneric},
        \syntaxref{defmacro}, \defopref{defclass}, \syntaxref{defun}.
    \end{flushleft}
    All these bindings are immutable.  \defopref{deflocal} creates a mutable
    top-lexical binding.  All such bindings are referenced by {\em variables}
    and those made by \defopref{deflocal} are modified by \specopref{setq}.
