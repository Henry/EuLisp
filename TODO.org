...
*** DONE Change =equal= generic function to a function which calls =binary==
    CLOSED: [2010-05-10 Mon 22:05]
    Note that the special case of (equal () ()) must be handled so that it
    returns =t=.
* EuLisp Definition
...
*** DONE Clarify the wording concerning the relationship between =nil= and =()=
    ...
    State that (eq 'nil '()) => t.
*** TODO Change =equal= methods to =binary==
*** TODO Change =equal= generic function to a function which calls =binary==
    Note that the special case of (equal () ()) must be handled so that it
    returns =t=.
