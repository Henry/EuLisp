...
*** TODO Complete support for eval
    Following the analysis in the report "Modules, Macros and Lisp", Queinnec
    and Padget, it is proposed to add for =eval= in EuLisp with the following
    operators:
    + Add additional module argument to =eval= to set the environment in which
      the eval evaluates.
    + Create =eval/cm= which calls =eval= with the current module as the second
      argument to evaluate in the current environment.
