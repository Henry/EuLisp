...
/* LispRef c_string_as_eul_string(char * str) */
/* { */
/*   return c_strn_as_eul_str(str, strlen(str)); */
/* } */
...
LispRef eul_c_str_as_eul_symbol(LispRef str)
{
    ...
    /*   tmp_str = (char *) gc_malloc(strlen((char *) str) + 1); */
...
LispRef eul_init_symbol(LispRef sym)
{
    ...
    str = eul_symbol_as_c_string(sym);
    res = eul_fast_table_ref(PGLOBAL(glob_symbols), str);
    if (eul_null(res))
    {
...
LispRef eul_init_keyword(LispRef key)
{
    ...
    str = eul_keyword_as_c_string(key);
    res = eul_fast_table_ref(PGLOBAL(glob_keywords), str);
    if (eul_null(res))
    {
...
char *eul_str_copy(char *str)
{
    ...
    res = (char *)gc_malloc(strlen(str) + 1);
    strcpy(res, str);
    return res;
}
...
LispRef eul_dyn_binding_ref(char *module_name, int i)
{
    ...
    bindings = eul_fast_table_ref(eul_modules, module_name);
...
int eul_sprintf_string(char *buf, int buf_offset, int n, int x_offset,
char *str, char *x)
...
LispRef eul_double_as_eul_double_ref(LispRef x)
{
    ...
    ptr = (double *)gc_malloc(sizeof(double *));
...
LispRef eul_time_start()
{
    ...
    if ((r = (int)times(&buffer)) == -1)
        return eul_nil;
    u = (int)buffer.tms_utime;
    s = (int)buffer.tms_stime;
...
LispRef eul_time_stop(LispRef x)
{
    ...
    u = (int)buffer.tms_utime;
    s = (int)buffer.tms_stime;
    old_r = eul_int_as_c_int(TIMES_REAL(x));
    old_u = eul_int_as_c_int(TIMES_USER(x));
    old_s = eul_int_as_c_int(TIMES_SYS(x));
