/** ----------------------------------------------------------------------- **
 **                 Generated by EuLisp System 'youtoo'
 ** ----------------------------------------------------------------------- **
 **  Description: C source file of EuLisp module i-error
 **  Copyright: See file i-error.em
 ** ----------------------------------------------------------------------- **/

#include "eulisp.h"


/* Imported modules */
extern void initialize_module_i_level1();
extern void initialize_module_i_param();
extern void initialize_module_i_notify();
extern LispRef i_level1_bindings[];
extern LispRef format_bindings[];
extern LispRef stream2_bindings[];
extern LispRef mop_meth_bindings[];
extern LispRef boot_bindings[];
extern LispRef i_param_bindings[];
extern LispRef mop_gf_bindings[];
extern LispRef mop_class_bindings[];
extern LispRef boot1_bindings[];
extern LispRef i_notify_bindings[];
extern LispRef condition_bindings[];

/* Module bindings with size 30 */
LispRef i_error_bindings[30];

/* Foreign functions */

/* Initialize module only once */
static int is_initialized = 0;

/* Initialize module i-error */
void initialize_module_i_error()
{
  if (is_initialized) return;
  initialize_module_i_level1();
  initialize_module_i_param();
  initialize_module_i_notify();
  eul_fast_table_set(eul_modules,"i_error",(LispRef) i_error_bindings);
  is_initialized = 1;
  {
  /* Declarations */
  LispRef sym_175, sym_174, sym_173, G00172, G00170, G00168, G00164, sym_162, sym_161, sym_160, key_158, key_157, key_156, sym_155, key_154, key_153, sym_152, key_151, G00150, G00148, G00146, G00144;

  /* Code vector and literal definitions */
  /* Byte-vector with size: 7 is_init: 0 index: 6 binding: (method-generic-print) */
  static const void *G00143[] = {I(ab,1c,82,24),B(i_error ,5),I(08,1d,82,24),B(condition ,8),I(08,24,00,00),B(i_notify ,6),I(3d,02,02,00)};

  /* Byte-vector with size: 3 is_init: 0 index: 7 binding: (method-get-ct-error-condition-class) */
  static const void *G00145[] = {I(aa,24,00,00),B(i_error ,5),I(45,01,00,00)};

  /* Byte-vector with size: 4 is_init: 0 index: 8 binding: (setter-ct-error-value) */
  static const void *G00147[] = {I(ab,1c,82,1d),I(24,00,00,00),B(i_error ,5),I(09,45,02,00)};

  eul_allocate_static_cons(cons_159, NULL, NULL);
  /* Byte-vector with size: 105 is_init: 0 index: 21 binding: top-level */
  static const void *G00149[] = {I(a9,24,00,00),B(condition ,8),I(24,00,00,00),B(boot1 ,25),I(3c,01,23,00),B(i_error ,9),I(23,00,00,00),B(i_error ,10),I(23,00,00,00),B(i_error ,11),I(23,00,00,00),B(i_error ,12),I(24,00,00,00),B(boot1 ,25),I(3c,04,1b,24),B(boot1 ,25),I(3c,01,24,00),B(mop_class ,71),I(23,00,00,00),B(i_error ,9),I(23,00,00,00),B(i_error ,13),I(23,00,00,00),B(i_error ,14),I(1f,06,23,00),B(i_error ,15),I(1f,06,23,00),B(i_error ,16),I(23,00,00,00),B(i_error ,17),I(24,00,00,00),B(mop_gf ,2),I(3c,09,1b,89),B(i_error ,5),I(2a,24,00,00),B(boot1 ,41),I(24,00,00,00),B(boot1 ,41),I(3c,01,24,00),B(i_error ,3),I(23,00,00,00),B(i_error ,18),I(23,00,00,00),B(i_error ,8),I(3b,02,1d,3c),I(02,2a,24,00),B(i_error ,5),I(2a,24,00,00),B(i_param ,3),I(26,00,00,00),I(00,00,00,03),I(02,83,86,24),B(boot1 ,39),I(3c,02,24,00),B(boot1 ,25),I(3c,00,24,00),B(i_param ,3),I(26,00,00,00),I(00,00,00,04),I(02,1c,1c,24),B(boot ,7),I(3c,02,1f,04),I(1f,04,23,00),B(i_error ,19),I(23,00,00,00),B(i_error ,7),I(3b,01,1f,03),I(24,00,00,00),B(mop_meth ,3),I(3c,04,24,00),B(i_param ,3),I(1c,24,00,00),B(mop_meth ,5),I(3c,02,2a,24),B(stream2 ,30),I(26,00,00,00),I(00,00,00,03),I(02,84,24,00),B(i_error ,5),I(24,00,00,00),B(stream2 ,20),I(24,00,00,00),B(boot1 ,39),I(3c,03,24,00),B(boot1 ,25),I(3c,00,24,00),B(stream2 ,30),I(26,00,00,00),I(00,00,00,04),I(02,1c,1c,24),B(boot ,7),I(3c,02,1f,04),I(1f,04,23,00),B(i_error ,20),I(23,00,00,00),B(i_error ,6),I(3b,02,1f,03),I(24,00,00,00),B(mop_meth ,3),I(3c,04,24,00),B(stream2 ,30),I(1c,24,00,00),B(mop_meth ,5),I(3d,02,11,45),I(11,00,00,00)};

  eul_allocate_static_string(str_165, "*** TOTAL NUMBER OF WARNINGS: ~a\n", 33);
  eul_allocate_static_string(str_166, "*** TOTAL NUMBER OF ERRORS: ~a\n", 31);
  /* Byte-vector with size: 41 is_init: 0 index: 24 binding: ct-exit */
  static const void *G00163[] = {I(a8,1b,34,00),I(00,00,00,0e),I(1b,10,32,00),I(00,00,00,07),I(86,24,00,00),B(i_param ,40),I(82,19,1b,34),I(00,00,00,0d),I(86,32,00,00),I(00,00,00,29),I(24,00,00,00),B(stream2 ,9),I(23,00,00,00),B(i_error ,22),I(24,00,00,00),B(i_param ,40),I(24,00,00,00),B(format ,4),I(3c,03,2a,24),B(i_param ,46),I(82,19,1b,34),I(00,00,00,0d),I(86,32,00,00),I(00,00,00,40),I(24,00,00,00),B(stream2 ,9),I(23,00,00,00),B(i_error ,23),I(24,00,00,00),B(i_param ,46),I(24,00,00,00),B(format ,4),I(3c,03,2a,1f),I(03,34,00,00),I(00,00,00,0f),I(86,32,00,00),I(00,00,00,0c),I(85,20,04,1f),I(03,2a,1d,24),B(boot1 ,20),I(3d,01,04,00)};

  /* Byte-vector with size: 3 is_init: 0 index: 25 binding: ct-error-value */
  static const void *G00167[] = {I(aa,82,24,00),B(i_error ,5),I(08,45,00,00)};

  /* Byte-vector with size: 12 is_init: 0 index: 26 binding: ct-error */
  static const void *G00169[] = {I(43,fd,24,00),B(format ,2),I(1d,1d,24,00),B(boot ,5),I(3c,03,24,00),B(i_error ,5),I(1c,23,00,00),B(i_error ,12),I(1f,06,24,00),B(boot ,12),I(3d,04,04,45),I(04,00,00,00)};

  /* Byte-vector with size: 43 is_init: 1 index: 0 binding: initialize-i-error */
  static const void *G00171[] = {I(87,25,00,00),B(i_error ,1),I(24,00,00,00),B(i_notify ,1),I(3e,0b,24,00),B(i_notify ,0),I(3c,00,21,01),I(24,00,00,00),B(i_param ,1),I(3e,0b,24,00),B(i_param ,0),I(3c,00,21,01),I(24,00,00,00),B(i_level1 ,1),I(3e,0b,24,00),B(i_level1 ,0),I(3c,00,21,01),I(86,25,00,00),B(i_error ,5),I(23,00,00,00),B(i_error ,13),I(23,00,00,00),B(i_error ,26),I(3b,fd,25,00),B(i_error ,4),I(23,00,00,00),B(i_error ,27),I(23,00,00,00),B(i_error ,25),I(3b,01,25,00),B(i_error ,3),I(23,00,00,00),B(i_error ,28),I(23,00,00,00),B(i_error ,24),I(3b,ff,25,00),B(i_error ,2),I(23,00,00,00),B(i_error ,29),I(23,00,00,00),B(i_error ,21),I(3b,00,3d,00),I(00,ac,00,00)};


  /* Initializations */
  eul_allocate_bytevector( G00144,G00143);
  eul_allocate_bytevector( G00146,G00145);
  eul_allocate_bytevector( G00148,G00147);
  eul_intern_keyword(key_151,"name");
  eul_intern_symbol(sym_152,"value");
  eul_intern_keyword(key_153,"keyword");
  eul_intern_keyword(key_154,"ct-error-value");
  eul_intern_symbol(sym_155,"ct-error");
  eul_intern_keyword(key_156,"direct-superclasses");
  eul_intern_keyword(key_157,"direct-slots");
  eul_intern_keyword(key_158,"direct-keywords");
  object_class(cons_159) = eul_static_cons_class;
  eul_car(cons_159) = key_154;
  eul_cdr(cons_159) = eul_nil;
  eul_intern_symbol(sym_160,"(setter ct-error-value)");
  eul_intern_symbol(sym_161,"(method get-ct-error-condition-class)");
  eul_intern_symbol(sym_162,"(method generic-print)");
  eul_allocate_bytevector( G00150,G00149);
  object_class(str_165) = eul_static_string_class;
  object_class(str_166) = eul_static_string_class;
  eul_allocate_bytevector( G00164,G00163);
  eul_allocate_bytevector( G00168,G00167);
  eul_allocate_bytevector( G00170,G00169);
  eul_intern_symbol(sym_173,"ct-error-value");
  eul_intern_symbol(sym_174,"ct-exit");
  eul_intern_symbol(sym_175,"top-level");
  eul_allocate_bytevector( G00172,G00171);

  /* Set local bindings */
  {
    int i;
    for (i = 2; i < 6; i++)
      i_error_bindings[i] = eul_nil;
  }

  i_error_bindings[ 6] = G00144;
  i_error_bindings[ 7] = G00146;
  i_error_bindings[ 8] = G00148;
  i_error_bindings[ 9] = key_151;
  i_error_bindings[ 10] = sym_152;
  i_error_bindings[ 11] = key_153;
  i_error_bindings[ 12] = key_154;
  i_error_bindings[ 13] = sym_155;
  i_error_bindings[ 14] = key_156;
  i_error_bindings[ 15] = key_157;
  i_error_bindings[ 16] = key_158;
  i_error_bindings[ 17] = cons_159;
  i_error_bindings[ 18] = sym_160;
  i_error_bindings[ 19] = sym_161;
  i_error_bindings[ 20] = sym_162;
  i_error_bindings[ 21] = G00150;
  i_error_bindings[ 22] = str_165;
  i_error_bindings[ 23] = str_166;
  i_error_bindings[ 24] = G00164;
  i_error_bindings[ 25] = G00168;
  i_error_bindings[ 26] = G00170;
  i_error_bindings[ 1] = eul_nil;
  i_error_bindings[ 27] = sym_173;
  i_error_bindings[ 28] = sym_174;
  i_error_bindings[ 29] = sym_175;
  eul_allocate_lambda( i_error_bindings[0], "initialize-i-error", 0, G00172);

  }
}


/* eof */
