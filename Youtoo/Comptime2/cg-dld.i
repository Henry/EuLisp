...
(definterface cg-dld
  (import (i-all sx-obj sx-node ex-import ex-syntax i-modify cg-interf p-env)
   ...
   local-literals (
    (top-level . 64)
    (make-module-env . 63)
    (dynamic-load-module . 62)
    (as-dynamic-binding . 61)
    (create-default-modules . 60)
    (module-loaded? . 59)
    (dynamic-binding-ref . 58)
    (dynamic-initialize-local-literals . 57)
    (dynamic-binding-set . 56)
    ("binding ~a not accessable in module ~a" . 54)
    ("binding ~a not available in module ~a" . 53)
    ("binding ~a not available in module ~a" . 49)
    (syntax-0 . 46)
    (level-1 . 45)
    (level-0 . 44)
    (math . 43)
