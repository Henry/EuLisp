/** ----------------------------------------------------------------------- **
 **                 Generated by EuLisp System 'youtoo'
 ** ----------------------------------------------------------------------- **
 **  Description: C source file of EuLisp module cg-dld
 **  Copyright: See file cg-dld.em
 ** ----------------------------------------------------------------------- **/

#include "eulisp.h"


/* Imported modules */
extern void initialize_module_i_all();
extern void initialize_module_sx_obj();
extern void initialize_module_sx_node();
extern void initialize_module_ex_import();
extern void initialize_module_ex_syntax();
extern void initialize_module_i_modify();
extern void initialize_module_cg_interf();
extern void initialize_module_p_env();
extern LispRef ex_syntax_bindings[];
extern LispRef ex_import_bindings[];
extern LispRef sx_obj_bindings[];
extern LispRef i_all_bindings[];
extern LispRef mop_gf_bindings[];
extern LispRef mop_class_bindings[];
extern LispRef sx_node_bindings[];
extern LispRef stream_bindings[];
extern LispRef sx_obj1_bindings[];
extern LispRef i_modify_bindings[];
extern LispRef string_bindings[];
extern LispRef cg_interf_bindings[];
extern LispRef table_bindings[];
extern LispRef mop_meth_bindings[];
extern LispRef boot_bindings[];
extern LispRef boot1_bindings[];
extern LispRef p_env_bindings[];
extern LispRef i_param_bindings[];
extern LispRef i_notify_bindings[];
extern LispRef format_bindings[];
extern LispRef dynamic_bindings[];

/* Module bindings with size 63 */
LispRef cg_dld_bindings[63];

/* Foreign functions */
static LispRef ff_stub_eul_initialize_level1_tables5713 (Stack *reg_value_stack, LispRef *sreg_value_sp, LispRef *sreg_value_sb)
{
  LispRef res;

  FF_RES_CONVERT6(res,eul_initialize_level1_tables());
  return res;
}

static LispRef ff_stub_eul_dyn_binding_ref5714 (Stack *reg_value_stack, LispRef *sreg_value_sp, LispRef *sreg_value_sb)
{
  LispRef G005784, G005785, res;

  POPVAL1(G005785);
  POPVAL1(G005784);
  FF_RES_CONVERT6(res,eul_dyn_binding_ref(FF_ARG_CONVERT3(G005784), FF_ARG_CONVERT0(G005785)));
  return res;
}

static LispRef ff_stub_eul_dyn_binding_set5715 (Stack *reg_value_stack, LispRef *sreg_value_sp, LispRef *sreg_value_sb)
{
  LispRef G005786, G005787, G005788, res;

  POPVAL1(G005788);
  POPVAL1(G005787);
  POPVAL1(G005786);
  FF_RES_CONVERT6(res,eul_dyn_binding_set(FF_ARG_CONVERT3(G005786), FF_ARG_CONVERT0(G005787), FF_ARG_CONVERT8(G005788)));
  return res;
}

static LispRef ff_stub_eul_dyn_load_module5716 (Stack *reg_value_stack, LispRef *sreg_value_sp, LispRef *sreg_value_sb)
{
  LispRef G005789, G005790, res;

  POPVAL1(G005790);
  POPVAL1(G005789);
  FF_RES_CONVERT0(res,eul_dyn_load_module(FF_ARG_CONVERT3(G005789), FF_ARG_CONVERT3(G005790)));
  return res;
}

static LispRef ff_stub_eul_module_name_as_C_module_name_string5717 (Stack *reg_value_stack, LispRef *sreg_value_sp, LispRef *sreg_value_sb)
{
  LispRef G005791, res;

  POPVAL1(G005791);
  FF_RES_CONVERT3(res,eul_module_name_as_C_module_name_string(FF_ARG_CONVERT8(G005791)));
  return res;
}


/* Initialize module only once */
static int is_initialized = 0;

/* Initialize module cg-dld */
void initialize_module_cg_dld()
{
  if (is_initialized) return;
  initialize_module_i_all();
  initialize_module_sx_obj();
  initialize_module_sx_node();
  initialize_module_ex_import();
  initialize_module_ex_syntax();
  initialize_module_i_modify();
  initialize_module_cg_interf();
  initialize_module_p_env();
  eul_fast_table_set(eul_modules,"cg_dld",(LispRef) cg_dld_bindings);
  is_initialized = 1;
  {
  /* Declarations */
  LispRef sym_5783, sym_5782, sym_5781, sym_5780, sym_5779, sym_5778, sym_5777, sym_5776, sym_5775, G005774, G005770, G005768, G005766, G005763, G005761, sym_5759, sym_5758, key_5757, G005755, sym_5752, sym_5751, G005750, G005742, sym_5740, G005739, G005737, G005735, G005733, G005731, sym_5729, G005728, sym_5725, sym_5724, sym_5722, G005721, G005719;

  /* Code vector and literal definitions */
  /* Byte-vector with size: 6 is_init: 0 index: 15 binding: anonymous */
  static const void *G005718[] = {I(a9,83,24,00),B(dynamic ,8),I(3c,01,2a,83),I(24,00,00,00),B(dynamic ,8),I(3d,01,00,00)};

  eul_allocate_static_string(str_5723, "  ~a", 4);
  eul_allocate_static_string(str_5726, "Loading syntax module ~a ...", 28);
  /* Byte-vector with size: 44 is_init: 0 index: 21 binding: (method-load-syntax-module) */
  static const void *G005720[] = {I(aa,23,00,00),B(cg_dld ,16),I(24,00,00,00),B(dynamic ,3),I(3c,01,23,00),B(cg_dld ,17),I(1c,24,00,00),B(format ,2),I(3c,02,23,00),B(cg_dld ,16),I(1c,24,00,00),B(dynamic ,2),I(3c,02,2a,23),B(cg_dld ,18),I(24,00,00,00),B(dynamic ,3),I(3c,01,23,00),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,15),I(3b,00,1c,0f),I(23,00,00,00),B(cg_dld ,18),I(1c,24,00,00),B(dynamic ,2),I(3c,02,2a,23),B(cg_dld ,20),I(1f,05,24,00),B(i_notify ,3),I(3c,02,2a,1f),I(04,89,00,00),B(i_param ,61),I(2a,1f,04,24),B(cg_dld ,3),I(3c,01,1f,05),I(1c,24,00,00),B(p_env ,9),I(3c,02,83,24),B(dynamic ,8),I(3c,01,2a,83),I(24,00,00,00),B(dynamic ,8),I(3c,01,2a,1b),I(45,07,00,00)};

  /* Byte-vector with size: 30 is_init: 0 index: 23 binding: top-level */
  static const void *G005727[] = {I(a9,24,00,00),B(i_param ,5),I(26,00,00,00),I(00,00,00,03),I(02,83,86,24),B(boot1 ,40),I(3c,02,24,00),B(boot1 ,25),I(3c,00,24,00),B(i_param ,5),I(26,00,00,00),I(00,00,00,04),I(02,1c,1c,24),B(boot ,7),I(3c,02,1f,04),I(1f,04,23,00),B(cg_dld ,22),I(23,00,00,00),B(cg_dld ,21),I(3b,01,1f,03),I(24,00,00,00),B(mop_meth ,3),I(3c,04,24,00),B(i_param ,5),I(1c,24,00,00),B(mop_meth ,5),I(3c,02,2a,24),B(cg_dld ,5),I(3d,00,06,45),I(06,00,00,00)};

  /* Byte-vector with size: 14 is_init: 0 index: 24 binding: anonymous */
  static const void *G005730[] = {I(ab,1c,34,00),I(00,00,00,2e),I(24,00,00,00),B(table ,5),I(24,00,00,00),B(boot1 ,42),I(3c,01,47,00),I(00,1f,03,1f),I(03,1f,03,3c),I(03,2a,1c,22),I(01,32,00,00),I(00,00,00,0a),I(47,00,00,45),I(02,00,00,00)};

  /* Byte-vector with size: 4 is_init: 0 index: 25 binding: anonymous */
  static const void *G005732[] = {I(aa,47,00,00),I(1c,24,00,00),B(table ,5),I(3d,02,01,00)};

  /* Byte-vector with size: 16 is_init: 0 index: 26 binding: make-module-env */
  static const void *G005734[] = {I(aa,46,01,1b),I(48,00,00,23),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,25),I(3b,01,24,00),B(boot1 ,42),I(24,00,00,00),B(boot1 ,42),I(3c,01,1c,23),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,24),I(3b,02,1d,3c),I(02,2a,1c,45),I(03,00,00,00)};

  /* Byte-vector with size: 10 is_init: 0 index: 27 binding: anonymous */
  static const void *G005736[] = {I(aa,1b,24,00),B(cg_dld ,6),I(3c,01,1b,34),I(00,00,00,0d),I(86,32,00,00),I(00,00,00,12),I(1c,24,00,00),B(cg_dld ,3),I(3d,01,02,45),I(02,00,00,00)};

  /* Byte-vector with size: 19 is_init: 0 index: 29 binding: anonymous */
  static const void *G005738[] = {I(a9,47,00,00),I(23,00,00,00),B(cg_dld ,28),I(50,1b,34,00),I(00,00,00,0e),I(86,32,00,00),I(00,00,00,22),I(24,00,00,00),B(i_param ,19),I(24,00,00,00),B(boot1 ,42),I(3c,01,47,00),I(00,86,1d,3c),I(02,22,01,2a),I(47,00,01,89),B(i_param ,20),I(2a,83,24,00),B(dynamic ,8),I(3d,01,01,00)};

  eul_allocate_static_string(str_5743, ".i", 2);
  eul_allocate_static_string(str_5744, ".c", 2);
  eul_allocate_static_string(str_5745, "Dynamically linking module ~a ...", 33);
  eul_allocate_static_string(str_5746, "initialize literals module ~a ...", 33);
  eul_allocate_static_string(str_5747, "run module ~a", 13);
  eul_allocate_static_string(str_5748, "module ~a can't be loaded correctly", 35);
  /* Byte-vector with size: 176 is_init: 0 index: 36 binding: dynamic-load-module */
  static const void *G005741[] = {I(43,fe,46,02),I(1c,48,00,00),I(24,00,00,00),B(i_param ,20),I(1b,48,00,01),I(23,00,00,00),B(cg_dld ,18),I(24,00,00,00),B(dynamic ,3),I(3c,01,23,00),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,29),I(3b,00,1c,0f),I(23,00,00,00),B(cg_dld ,18),I(1c,24,00,00),B(dynamic ,2),I(3c,02,2a,1f),I(03,12,1b,34),I(00,00,00,15),I(47,00,00,24),B(cg_dld ,6),I(3c,01,32,00),I(00,00,00,07),I(86,1b,34,00),I(00,00,00,32),I(47,00,00,24),B(p_env ,14),I(3c,01,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,11),I(47,00,00,24),B(cg_interf ,12),I(3c,01,1b,22),I(02,32,00,00),I(00,00,01,e3),I(86,89,00,00),B(i_param ,20),I(2a,1f,05,12),I(1b,34,00,00),I(00,00,00,7b),I(47,00,00,24),B(p_env ,14),I(3c,01,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,5b),I(47,00,00,7d),I(1b,34,00,00),I(00,00,00,0f),I(1b,32,00,00),I(00,00,00,0c),I(47,00,00,82),I(02,1b,23,00),B(cg_dld ,30),I(24,00,00,00),B(string ,11),I(3c,02,24,00),B(i_modify ,7),I(3c,01,1b,34),I(00,00,00,15),I(47,00,00,24),B(cg_interf ,12),I(3c,01,32,00),I(00,00,00,10),I(47,00,00,24),B(i_param ,18),I(3c,01,22,03),I(22,01,32,00),I(00,00,00,10),I(47,00,00,24),B(i_param ,18),I(3c,01,47,00),I(01,89,00,00),B(i_param ,20),I(2a,1b,26,00),I(00,00,00,03),I(24,00,00,00),B(sx_obj1 ,59),I(08,1c,84,24),B(sx_obj1 ,59),I(08,2a,47,00),I(00,7d,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,0c),I(47,00,00,82),I(02,23,00,00),B(cg_dld ,31),I(24,00,00,00),B(string ,11),I(3c,02,47,00),I(00,41,00,00),B(cg_dld ,14),I(22,01,24,00),B(stream ,17),I(1d,24,00,00),B(i_param ,16),I(24,00,00,00),B(boot ,5),I(3c,03,10,23),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,27),I(3b,01,1f,05),I(24,00,00,00),B(boot ,16),I(3c,02,2a,23),B(cg_dld ,32),I(47,00,00,24),B(i_notify ,3),I(3c,02,2a,1c),I(1c,41,00,00),B(cg_dld ,13),I(22,02,82,1c),I(1a,1b,34,00),I(00,00,00,8a),I(1f,07,1d,1c),I(26,00,00,00),I(00,00,00,12),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1c,26,00,00),I(00,00,01,00),I(14,1f,08,1c),I(1c,26,00,00),I(00,00,00,11),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(23,00,00,00),B(cg_dld ,33),I(47,00,00,24),B(i_notify ,4),I(3c,02,2a,1f),I(08,47,00,00),I(24,00,00,00),B(cg_dld ,8),I(3c,02,2a,23),B(cg_dld ,34),I(47,00,00,24),B(i_notify ,4),I(3c,02,2a,1f),I(04,82,41,00),B(cg_dld ,11),I(22,02,1b,3c),I(00,2a,1f,09),I(22,02,32,00),I(00,00,00,18),I(23,00,00,00),B(cg_dld ,35),I(47,00,00,24),B(boot ,12),I(3c,02,22,09),I(47,00,00,23),B(cg_dld ,28),I(50,1b,34,00),I(00,00,00,0e),I(86,32,00,00),I(00,00,00,22),I(24,00,00,00),B(i_param ,19),I(24,00,00,00),B(boot1 ,42),I(3c,01,47,00),I(00,86,1d,3c),I(02,22,01,2a),I(47,00,01,89),B(i_param ,20),I(2a,83,24,00),B(dynamic ,8),I(3c,01,2a,1c),I(45,09,00,00)};

  eul_allocate_static_string(str_5753, "binding ~a not accessable in module ~a", 38);
  /* Byte-vector with size: 46 is_init: 0 index: 40 binding: as-dynamic-binding */
  static const void *G005749[] = {I(aa,1b,83,24),B(sx_obj1 ,42),I(08,1c,26,00),I(00,00,00,05),I(24,00,00,00),B(sx_obj1 ,42),I(08,1b,7c,1b),I(34,00,00,00),I(00,00,00,10),I(1c,32,00,00),I(00,00,00,18),I(1c,26,00,00),I(00,00,00,15),I(24,00,00,00),B(sx_obj1 ,59),I(08,1f,04,23),B(cg_dld ,37),I(24,00,00,00),B(sx_node ,20),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,19),I(1f,05,23,00),B(cg_dld ,38),I(24,00,00,00),B(sx_node ,20),I(3c,02,1b,34),I(00,00,00,31),I(1f,06,26,00),I(00,00,00,06),I(24,00,00,00),B(sx_obj1 ,42),I(08,23,00,00),B(cg_dld ,39),I(1c,1f,05,24),B(boot ,12),I(3d,03,08,22),I(01,32,00,00),I(00,00,00,1d),I(1d,41,00,00),B(cg_dld ,14),I(22,01,1b,1f),I(07,41,00,00),B(cg_dld ,11),I(22,03,45,07)};

  eul_allocate_static_string(str_5756, "Create default modules ...", 26);
  /* Byte-vector with size: 124 is_init: 0 index: 45 binding: create-default-modules */
  static const void *G005754[] = {I(a9,23,00,00),B(cg_dld ,41),I(24,00,00,00),B(i_notify ,4),I(3c,01,2a,41),B(cg_dld ,10),I(1b,82,02,1c),I(83,02,1d,84),I(02,24,00,00),B(boot ,26),I(3c,01,1f,03),I(26,00,00,00),I(00,00,00,03),I(02,24,00,00),B(mop_class ,5),I(23,00,00,00),B(cg_dld ,42),I(1d,24,00,00),B(mop_gf ,2),I(3c,03,1f,05),I(26,00,00,00),I(00,00,00,04),I(02,24,00,00),B(mop_class ,5),I(23,00,00,00),B(cg_dld ,42),I(1d,24,00,00),B(mop_gf ,2),I(3c,03,1d,24),B(sx_node ,3),I(3c,01,1c,24),B(sx_node ,3),I(3c,01,23,00),B(cg_dld ,28),I(24,00,00,00),B(sx_node ,3),I(3c,01,23,00),B(cg_dld ,43),I(24,00,00,00),B(sx_node ,3),I(3c,01,2a,23),B(cg_dld ,44),I(24,00,00,00),B(sx_node ,3),I(3c,01,1f,0a),I(24,00,00,00),B(cg_dld ,2),I(3c,01,1f,0a),I(24,00,00,00),B(cg_dld ,2),I(3c,01,1f,05),I(86,1c,26,00),I(00,00,00,12),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,04,86,1c),I(26,00,00,00),I(00,00,00,12),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,05,1d,1c),I(26,00,00,00),I(00,00,00,0f),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,04,1c,1c),I(26,00,00,00),I(00,00,00,0f),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1d,1d,1c,26),I(00,00,00,10),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1d,1d,1c,26),I(00,00,00,0f),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1d,1c,1c,26),I(00,00,00,0e),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,03,84,1c),I(26,00,00,00),I(00,00,00,12),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,03,26,00),I(00,00,04,00),I(1c,26,00,00),I(00,00,00,11),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,03,1d,1c),I(26,00,00,00),I(00,00,00,10),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,03,1d,1c),I(26,00,00,00),I(00,00,00,0f),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,03,1c,1c),I(26,00,00,00),I(00,00,00,0e),I(1d,24,00,00),B(sx_obj1 ,59),I(09,22,02,2a),I(1f,05,1f,0b),I(1c,83,1d,24),B(sx_obj1 ,59),I(09,45,10,00)};

  /* Byte-vector with size: 5 is_init: 0 index: 46 binding: module-loaded? */
  static const void *G005760[] = {I(aa,41,00,00),B(cg_dld ,14),I(22,01,82,41),B(cg_dld ,11),I(45,02,00,00)};

  eul_allocate_static_string(str_5764, "binding ~a not available in module ~a", 37);
  /* Byte-vector with size: 37 is_init: 0 index: 48 binding: dynamic-binding-ref */
  static const void *G005762[] = {I(ab,1b,24,00),B(p_env ,14),I(3c,01,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,11),I(1c,24,00,00),B(cg_interf ,12),I(3c,01,1f,03),I(1c,24,00,00),B(p_env ,10),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,55),I(1f,04,1d,24),B(p_env ,7),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,3b),I(1f,05,1f,03),I(24,00,00,00),B(p_env ,19),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,1d),I(23,00,00,00),B(cg_dld ,47),I(1f,07,1f,07),I(24,00,00,00),B(boot ,12),I(3c,03,22,01),I(22,01,24,00),B(cg_dld ,4),I(3d,01,05,00)};

  /* Byte-vector with size: 13 is_init: 0 index: 49 binding: anonymous */
  static const void *G005765[] = {I(aa,1b,12,1b),I(34,00,00,00),I(00,00,00,10),I(86,32,00,00),I(00,00,00,25),I(1c,10,1b,11),I(1c,10,47,00),I(00,1d,1d,41),B(cg_dld ,12),I(22,03,2a,1f),I(04,11,47,00),I(01,3d,01,05),I(22,03,45,02)};

  /* Byte-vector with size: 16 is_init: 0 index: 50 binding: dynamic-initialize-local-literals */
  static const void *G005767[] = {I(ab,46,02,41),B(cg_dld ,14),I(22,01,1c,26),I(00,00,00,04),I(24,00,00,00),B(sx_obj1 ,59),I(08,1c,48,00),I(00,86,1b,48),I(00,01,23,00),B(cg_dld ,19),I(23,00,00,00),B(cg_dld ,49),I(3b,01,48,00),I(01,1c,47,00),I(01,3d,01,04),I(45,04,00,00)};

  eul_allocate_static_string(str_5771, "binding ~a not available in module ~a", 37);
  eul_allocate_static_string(str_5772, "binding ~a not accessable in module ~a", 38);
  /* Byte-vector with size: 68 is_init: 0 index: 53 binding: dynamic-binding-set */
  static const void *G005769[] = {I(43,03,1c,24),B(p_env ,14),I(3c,01,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,11),I(1d,24,00,00),B(cg_interf ,12),I(3c,01,1f,04),I(1c,24,00,00),B(p_env ,10),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,55),I(1f,05,1d,24),B(p_env ,7),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,3b),I(1f,06,1f,03),I(24,00,00,00),B(p_env ,19),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,1d),I(23,00,00,00),B(cg_dld ,51),I(1f,08,1f,08),I(24,00,00,00),B(boot ,12),I(3c,03,22,01),I(22,01,1b,83),I(24,00,00,00),B(sx_obj1 ,42),I(08,1c,26,00),I(00,00,00,05),I(24,00,00,00),B(sx_obj1 ,42),I(08,1d,23,00),B(cg_dld ,37),I(24,00,00,00),B(sx_node ,20),I(3c,02,1b,34),I(00,00,00,0d),I(1b,32,00,00),I(00,00,00,19),I(1f,03,23,00),B(cg_dld ,38),I(24,00,00,00),B(sx_node ,20),I(3c,02,1b,34),I(00,00,00,21),I(23,00,00,00),B(cg_dld ,52),I(1f,0b,1f,0b),I(24,00,00,00),B(boot ,12),I(3d,03,0b,32),I(00,00,00,1b),I(1d,41,00,00),B(cg_dld ,14),I(22,01,1b,1f),I(05,1f,0b,41),B(cg_dld ,12),I(22,04,45,0b)};

  /* Byte-vector with size: 96 is_init: 1 index: 0 binding: initialize-cg-dld */
  static const void *G005773[] = {I(87,25,00,00),B(cg_dld ,1),I(24,00,00,00),B(p_env ,1),I(3e,0b,24,00),B(p_env ,0),I(3c,00,21,01),I(24,00,00,00),B(cg_interf ,1),I(3e,0b,24,00),B(cg_interf ,0),I(3c,00,21,01),I(24,00,00,00),B(i_modify ,1),I(3e,0b,24,00),B(i_modify ,0),I(3c,00,21,01),I(24,00,00,00),B(ex_syntax ,1),I(3e,0b,24,00),B(ex_syntax ,0),I(3c,00,21,01),I(24,00,00,00),B(ex_import ,1),I(3e,0b,24,00),B(ex_import ,0),I(3c,00,21,01),I(24,00,00,00),B(sx_node ,1),I(3e,0b,24,00),B(sx_node ,0),I(3c,00,21,01),I(24,00,00,00),B(sx_obj ,1),I(3e,0b,24,00),B(sx_obj ,0),I(3c,00,21,01),I(24,00,00,00),B(i_all ,1),I(3e,0b,24,00),B(i_all ,0),I(3c,00,21,01),I(23,00,00,00),B(cg_dld ,54),I(23,00,00,00),B(cg_dld ,53),I(3b,03,25,00),B(cg_dld ,9),I(23,00,00,00),B(cg_dld ,55),I(23,00,00,00),B(cg_dld ,50),I(3b,02,25,00),B(cg_dld ,8),I(23,00,00,00),B(cg_dld ,56),I(23,00,00,00),B(cg_dld ,48),I(3b,02,25,00),B(cg_dld ,7),I(23,00,00,00),B(cg_dld ,57),I(23,00,00,00),B(cg_dld ,46),I(3b,01,25,00),B(cg_dld ,6),I(23,00,00,00),B(cg_dld ,58),I(23,00,00,00),B(cg_dld ,45),I(3b,00,25,00),B(cg_dld ,5),I(23,00,00,00),B(cg_dld ,59),I(23,00,00,00),B(cg_dld ,40),I(3b,01,25,00),B(cg_dld ,4),I(23,00,00,00),B(cg_dld ,60),I(23,00,00,00),B(cg_dld ,36),I(3b,fe,25,00),B(cg_dld ,3),I(23,00,00,00),B(cg_dld ,61),I(23,00,00,00),B(cg_dld ,26),I(3b,01,25,00),B(cg_dld ,2),I(23,00,00,00),B(cg_dld ,62),I(23,00,00,00),B(cg_dld ,23),I(3b,00,3d,00),I(00,ac,00,00)};


  /* Initializations */
  eul_allocate_bytevector( G005719,G005718);
  eul_intern_symbol(sym_5722,"*indent*");
  object_class(str_5723) = eul_static_string_class;
  eul_intern_symbol(sym_5724,"*clean-ups*");
  eul_intern_symbol(sym_5725,"anonymous");
  object_class(str_5726) = eul_static_string_class;
  eul_allocate_bytevector( G005721,G005720);
  eul_intern_symbol(sym_5729,"(method load-syntax-module)");
  eul_allocate_bytevector( G005728,G005727);
  eul_allocate_bytevector( G005731,G005730);
  eul_allocate_bytevector( G005733,G005732);
  eul_allocate_bytevector( G005735,G005734);
  eul_allocate_bytevector( G005737,G005736);
  eul_intern_symbol(sym_5740,"user");
  eul_allocate_bytevector( G005739,G005738);
  object_class(str_5743) = eul_static_string_class;
  object_class(str_5744) = eul_static_string_class;
  object_class(str_5745) = eul_static_string_class;
  object_class(str_5746) = eul_static_string_class;
  object_class(str_5747) = eul_static_string_class;
  object_class(str_5748) = eul_static_string_class;
  eul_allocate_bytevector( G005742,G005741);
  eul_intern_symbol(sym_5751,"opencoding");
  eul_intern_symbol(sym_5752,"ff");
  object_class(str_5753) = eul_static_string_class;
  eul_allocate_bytevector( G005750,G005749);
  object_class(str_5756) = eul_static_string_class;
  eul_intern_keyword(key_5757,"name");
  eul_intern_symbol(sym_5758,"math");
  eul_intern_symbol(sym_5759,"level0");
  eul_allocate_bytevector( G005755,G005754);
  eul_allocate_bytevector( G005761,G005760);
  object_class(str_5764) = eul_static_string_class;
  eul_allocate_bytevector( G005763,G005762);
  eul_allocate_bytevector( G005766,G005765);
  eul_allocate_bytevector( G005768,G005767);
  object_class(str_5771) = eul_static_string_class;
  object_class(str_5772) = eul_static_string_class;
  eul_allocate_bytevector( G005770,G005769);
  eul_intern_symbol(sym_5775,"dynamic-binding-set");
  eul_intern_symbol(sym_5776,"dynamic-initialize-local-literals");
  eul_intern_symbol(sym_5777,"dynamic-binding-ref");
  eul_intern_symbol(sym_5778,"module-loaded?");
  eul_intern_symbol(sym_5779,"create-default-modules");
  eul_intern_symbol(sym_5780,"as-dynamic-binding");
  eul_intern_symbol(sym_5781,"dynamic-load-module");
  eul_intern_symbol(sym_5782,"make-module-env");
  eul_intern_symbol(sym_5783,"top-level");
  eul_allocate_bytevector( G005774,G005773);

  /* Set local bindings */
  {
    int i;
    for (i = 2; i < 10; i++)
      cg_dld_bindings[i] = eul_nil;
  }

  cg_dld_bindings[ 10] = (LispRef) (LispRef (*) (Stack *, LispRef *, LispRef *)) ff_stub_eul_initialize_level1_tables5713;
  cg_dld_bindings[ 11] = (LispRef) (LispRef (*) (Stack *, LispRef *, LispRef *)) ff_stub_eul_dyn_binding_ref5714;
  cg_dld_bindings[ 12] = (LispRef) (LispRef (*) (Stack *, LispRef *, LispRef *)) ff_stub_eul_dyn_binding_set5715;
  cg_dld_bindings[ 13] = (LispRef) (LispRef (*) (Stack *, LispRef *, LispRef *)) ff_stub_eul_dyn_load_module5716;
  cg_dld_bindings[ 14] = (LispRef) (LispRef (*) (Stack *, LispRef *, LispRef *)) ff_stub_eul_module_name_as_C_module_name_string5717;
  cg_dld_bindings[ 15] = G005719;
  cg_dld_bindings[ 16] = sym_5722;
  cg_dld_bindings[ 17] = str_5723;
  cg_dld_bindings[ 18] = sym_5724;
  cg_dld_bindings[ 19] = sym_5725;
  cg_dld_bindings[ 20] = str_5726;
  cg_dld_bindings[ 21] = G005721;
  cg_dld_bindings[ 22] = sym_5729;
  cg_dld_bindings[ 23] = G005728;
  cg_dld_bindings[ 24] = G005731;
  cg_dld_bindings[ 25] = G005733;
  cg_dld_bindings[ 26] = G005735;
  cg_dld_bindings[ 27] = G005737;
  cg_dld_bindings[ 28] = sym_5740;
  cg_dld_bindings[ 29] = G005739;
  cg_dld_bindings[ 30] = str_5743;
  cg_dld_bindings[ 31] = str_5744;
  cg_dld_bindings[ 32] = str_5745;
  cg_dld_bindings[ 33] = str_5746;
  cg_dld_bindings[ 34] = str_5747;
  cg_dld_bindings[ 35] = str_5748;
  cg_dld_bindings[ 36] = G005742;
  cg_dld_bindings[ 37] = sym_5751;
  cg_dld_bindings[ 38] = sym_5752;
  cg_dld_bindings[ 39] = str_5753;
  cg_dld_bindings[ 40] = G005750;
  cg_dld_bindings[ 41] = str_5756;
  cg_dld_bindings[ 42] = key_5757;
  cg_dld_bindings[ 43] = sym_5758;
  cg_dld_bindings[ 44] = sym_5759;
  cg_dld_bindings[ 45] = G005755;
  cg_dld_bindings[ 46] = G005761;
  cg_dld_bindings[ 47] = str_5764;
  cg_dld_bindings[ 48] = G005763;
  cg_dld_bindings[ 49] = G005766;
  cg_dld_bindings[ 50] = G005768;
  cg_dld_bindings[ 51] = str_5771;
  cg_dld_bindings[ 52] = str_5772;
  cg_dld_bindings[ 53] = G005770;
  cg_dld_bindings[ 1] = eul_nil;
  cg_dld_bindings[ 54] = sym_5775;
  cg_dld_bindings[ 55] = sym_5776;
  cg_dld_bindings[ 56] = sym_5777;
  cg_dld_bindings[ 57] = sym_5778;
  cg_dld_bindings[ 58] = sym_5779;
  cg_dld_bindings[ 59] = sym_5780;
  cg_dld_bindings[ 60] = sym_5781;
  cg_dld_bindings[ 61] = sym_5782;
  cg_dld_bindings[ 62] = sym_5783;
  eul_allocate_lambda( cg_dld_bindings[0], "initialize-cg-dld", 0, G005774);

  }
}


/* eof */
