...
//
///-----------------------------------------------------------------------------
/// Title: Card and type description interface
///  Library: Xalloc
...
void initialize()
{
    ...
    for (register int cd = SMALLEST_CARDDSCR; cd < HIGHEST_CARDDSCR; cd++)
    {
...
int set_lowest_type_descriptor(TypeDscr value)
{
    if (!initialized)
    {
...
(
    ...
    register void *(*fcn)() // void* instead of void to be compatible with Apply
)
{
    if (!initialized)
    {
...
(
    register unsigned long t,
    register void *(*fcn) () // void* instead of void to be compatible to Apply
)
{
    if (!initialized)
    {
    ...
    register TypeDscr i = 1;
    for (; i < last_used_type; i++)
    {
...
int set_lowest_card_descriptor(CardDscr value)
{
    if (!initialized)
    {
...
)
{
    ...
    register CardDscr i = 0;
    for (; i < last_used_card; i++)
    {
...
unsigned long safe_get_type(register long *p)
{
    if (p_in_heap(p))
    {
...
char get_card_type(register CardDscr cd)
{
    #ifdef  USE_MTSS
    if (mt_card_d(cd))
    {
...
TypeDscr get_type_descriptor(register long *ptr)
{
    #ifdef USE_MTSS
...
)
{
    ...
    if (!ms_card_d(cd))
    {
        ...
        if (get_size_d(cd) > USERSPACE)
        {
            ...
            (
                c,
                (get_size_d(cd) - (mt_card_d(cd) ? 1 : 0))*sizeof(long)
            );
