...
//
///-----------------------------------------------------------------------------
/// Title: xalloc allocation functions
///  Library: Xalloc
...
///-----------------------------------------------------------------------------
// a function to be used in the next macro: builds an and returns pointer to the
// free list on card c
long *build_free_list(register card *c, register long *pt, register CardDscr cd)
{
...
void prepare_ss_card(CardDscr cd, card* c, int o)
{
    ...
    put_cardnum(c, 1);
    store_free_object(cd, build_free_list(c, (long *)(c + 1), cd));
    set_mask(c);
    clear_marks(c);
}
...
// returns pointer to free object of carddscr cd
...
// returns pointer to free object of carddscr cd, prepared with typedscr td
...
// returns pointer to free object of carddscr cd, with length i bytes
void *xalloc_stms(register CardDscr cd, register long i)
{
    // this is really something else, because any found object may be to short.
    // This causes the loop in this function and makes it in general much slower
    // than the both above. if def. fast_stms we prefer gc to long search times
    ...
    else
    {
        ...
        long sz1 = sz - (noc - 1)*byte2word(CARDSIZE);
